#!/usr/bin/env python3
"""Record the Pax Silica charts as a montage: one animated chart per beat.
...
Everything on screen is the live page reading the baked numbers, so the montage
...
async def main():
    ...
    run(["ffmpeg", "-y", "-i", src, "-vf", vf + ",palettegen=stats_mode=diff",
         "-update", "1", "-frames:v", "1", pal])
    run(["ffmpeg", "-y", "-i", src, "-i", pal, "-lavfi",
