...
The reserve replay clears the RTDOR-derived reserve books at the
...
per-resource reserve results (DIPCRF) provide a tighter check and the resource
schedule that a joint energy-and-reserve calculation would need.
...
codes; the REGION map below carries both spellings so the reconciliation does
not silently omit a region.
...
def derive_day(date: str, hour_files: list[tuple[str, str]]) -> dict:
    ...
    for g in GRIDS:
        for c in COMMODITIES:
            ...
            rtd_gap[g][c] = {
                "max_abs": round(max((abs(d) for d in diffs)), 1) if diffs else None,
                "mean": round(sum(diffs) / len(diffs), 1) if diffs else None,
            }
    populated = sum(
        1
        for h in range(24)
        if sched["luzon"]["Dr"][h] is not None and sched["luzon"]["Fr"][h] is not None
    )
    if populated < 20:
        raise RuntimeError(
    ...
    return {
        ...
        "note": (
            "Per-resource cleared reserve from IEMOP's DIPC reserve "
...
def derive(
    limit: int | None = None, frm: str = "2026-05-01", to: str | None = None
) -> int:
    ...
    for b64, name in listing:
        ...
        if m:
            ...
            if fhh == 0:
                fdate = (
                    datetime.strptime(fdate, "%Y%m%d") - timedelta(days=1)
                ).strftime("%Y%m%d")
