#!/usr/bin/env python3
"""Generate the connection limits for every announced load site (web/data/sites.json).
...
day, never on what a user types, so it belongs in the data build. What a user types is
...
def _limit(net, inj, bus, local) -> tuple[float | None, float]:
    ...
    report and the supported answer is nothing, not a crossing point extrapolated
    ...
    if s0 is None or s1 is None:
        return None, 0.0
    base = max(
        abs(s0["flows_mw"][bi]) / net["branches"][bi]["rating_mw"] for bi in local
    )
    if base >= 1.0:
        return 0.0, base
    best = None
...
def build_sites(date: str) -> dict:
    ...
    for site in _catalogue():
        ...
        local = [
            bi for bi, b in enumerate(all_branches) if b["a"] == bus or b["b"] == bus
        ]
        if not local:
            continue
        ...
        for drop in local:
            ...
            lim = (
                _limit(net, inj_by_hour[19], bus, local2)[0]
                if local2 and not cut
                else None
            )
        ...
        rows.append(
            {
                **site,
                ...
                "circuits": [
                    {
                        "names": all_branches[bi]["names"]
                        or [all_branches[bi]["kind"]],
                ...
                "linearity_max_error": (
                    round(max(abs(c - 1.0) for c in checks), 4) if checks else None
                ),
            }
        )
        print(
            f"  {site['name'][:44]:46} {placed['grid']:9} "
    ...
    return {
        ...
        "note": (
            ...
            "named an operating limit. The available megawatts depend on those "
            "assumed ratings. Site "
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    if not a.day:
        # the data build runs unattended, so pick the newest day the nodal archive has
    ...
    print(
        f"\nwrote web/data/sites.json: {res['n_sites']} sites, {n_radial} fed radially"
    )
