#!/usr/bin/env python3
"""Generate power-dispatch-studio web/data/ from the IEMOP archive + verified constants.

The site values are computed from archived IEMOP files in data/raw/ or from a
labeled constant with a primary source in pipeline/constants_ph.py. The
frontend reads these generated files.
...
def rows_of(path: str) -> list[dict]:
    with open(path, newline="", encoding="utf-8", errors="replace") as f:
        return [
            r for r in csv.DictReader(f) if any((v or "").strip() for v in r.values())
        ]
...
def assert_cols(path: str, rows: list[dict], needed: set[str]) -> None:
    ...
    if missing:
        raise SystemExit(
            f"SCHEMA DRIFT in {os.path.basename(path)}: "
...
def build_congestion() -> dict:
    ...
    for key in ("RTDCV", "DAPCV"):
        for path in dataset_files(key):
            ...
            rows = rows_of(path)
            assert_cols(
                path, rows, {"EQUIPMENT_NAME", "STATION_NAME", "CONGEST_TYPE", "PCT_MW"}
            )
            days_seen.add(day)
            for r in rows:
                ...
                # near-zero on ~96% of real-time rows because the dispatch holds
                # flow AT the limit. A line pinned at 100% of its limit is fully
                # bound with zero overload, so max_overload_mw alone reads the most
                ...
                for cid in corridor_ids(name):
                    ...
                    c["max_pct_of_limit"] = max(c.get("max_pct_of_limit", 0.0), pct)
    out = []
    for e in league.values():
        ...
        out.append(e)
    out.sort(
        key=lambda e: (-e["days"], -e["rtd_intervals"], -e["dap_rows"], e["equipment"])
    )
    ...
    for cid, c in corridors.items():
        receipts[cid] = {
            "days": len(c["days"]),
            "rtd_intervals": c["rtd_intervals"],
            "rtd_days": len(c["rtd_days"]),
            "dap_rows": c["dap_rows"],
            ...
            "matched_equipment": [
                {"name": k, "rows": v}
                for k, v in sorted(c["matched"].items(), key=lambda kv: -kv[1])
            ],
        }
    return {
...
def build_reliability() -> dict:
    ...
    for grid in GRIDS:
        ...
        totals[grid.lower()] = {
            ...
            "worst_curtailment": max(
                curt_days, key=lambda r: r["curtailed_mwh"], default=None
            ),
            "reserve_shortfall_days": len(short_days),
            "median_daily_min_reserve_slack_mw": round(
                median([r["reserve_slack_min_mw"] for r in rows_]), 1
            )
            if rows_
            else None,
        }
...
def build_price_load() -> dict:
    ...
    if best_day:
        ...
        rep = {
            "date": best_day,
            "series": {
                g: [{"t": t, "gen_mw": gen, "price": pr} for t, gen, pr in ds[g]]
                for g in ds
                if ds[g]
            },
        }

    # downsample the scatter so the generated file stays small (charts read the curve;
    ...
    return {
        "unit": "generation MW vs load-weighted price PhP/kWh, per 5-min interval",
...
def build_prices() -> dict:
    ...
    for grid in GRIDS:
        series[grid.lower()] = [
            round(sum(v) / len(v) / 1000, 3) if (v := daily.get((day, grid))) else None
            for day in days
        ]
    spread = []
    for i in range(len(days)):
        vals = [series[g.lower()][i] for g in GRIDS if series[g.lower()][i] is not None]
    ...
    def regime_stats(pred):
        ...
        for g in GRIDS:
            vals = [
                series[g.lower()][i] for i in keep if series[g.lower()][i] is not None
            ]
...
def build_hvdc() -> dict:
    ...
    return {
        ...
        "note": (
            "no HVDC limit events recorded in RTD in this window; "
            "the binding evidence for the links is in IEMOP's monthly "
            "reports"
            if limit_rows == 0
            else "per-interval limit series is a v1.1 layer"
        ),
    }
...
def build_outages() -> dict:
    ...
    name_col = next(
        (
            c
            for c in header
            if any(k in c.upper() for k in ("RESOURCE", "PLANT", "NAME", "FACILITY"))
        ),
        None,
    )
    if not name_col:
            ...
            rows_.append(
                {
                    ...
                    "max_abs_php_kwh": round(max(nonzero) / 1000, 3)
                    if nonzero
                    else 0.0,
                }
            )
...
def build_findings(cong, rel, prices, outs, named_dc_mw, n_dc) -> dict:
    ...
    ltc = next(
        (e for e in cong.get("league", []) if e["equipment"] == "LEYTE_TO_CEBU"), None
    )
    ...
    if cr and ltc and top:
        add(
            "corridor-named",
            "Recorded limit",
            f"Leyte-Cebu reached a binding limit on {top['days']} of "
            f"{cong['days_covered']} days",
            f"A row literally named LEYTE_TO_CEBU appears in the day-ahead "
            f"runs on {ltc['dap_days']} of {cong['days_covered']} days",
            f"IEMOP's congestion files name equipment that reached its limit. "
            f"The Tabango (Leyte) to Daanbantayan (Cebu) 230 kV lines had the "
            f"most days at a limit, {top['days']} of "
            f"{cong['days_covered']} archive days "
            f"across the day-ahead and real-time runs, including "
            f"{cr['rtd_intervals']} 5-minute real-time intervals on "
            f"{cr['rtd_days']} days. IEMOP's December 2025 report names the "
            f"same transmission path.",
    ...
    if mk and ad and P.get("max_spread"):
        add(
            ...
            f"P{P['max_spread']['php']}/kWh on {P['max_spread']['date']}. "
            "Limited transfer capacity lets regional prices separate when "
            "local supply and demand differ.",
...
def build_answers(cong, rel, prices, outs) -> dict:
    ...
    q1_stat = (
        f"{dict_fc['mw']:,} MW forecast by 2028 vs a "
        f"{margin:,} MW May 2026 system margin"
    )
    q1_blurb = (
        f"DICT's 2028 forecast alone equals "
    ...
    if short_days is not None and short_days > 0:
        q2_blurb += (
            f" Luzon scheduled reserves fell below the stated "
    ...
    spot_bill = (
        ...
        / 100
        * A["meralco_june2026_wesm_price_php_kwh"]
    )
    q3_blurb += (
...
def main() -> int:
    ...
    findings = build_findings(
        congestion, reliability, prices, outages, named_mw, n_named
    )
    ...
    grid_summary = build_grid(
        congestion.pop("league_full"), OUT, chokepoints=CHOKEPOINTS
    )
    ...
    with open(os.path.join(OUT, "grid.json"), "w") as fh:
        json.dump({"summary": grid_summary, "match_report": grid_report}, fh, indent=1)

    receipts = congestion.get("corridor_receipts") or {}
    ck = []
    for c in CHOKEPOINTS:
        props = {k: v for k, v in c.items() if k not in ("coords", "equipment_match")}
        if c["id"] in receipts:
            props["receipts"] = receipts[c["id"]]
        elif c["kind"] == "hvdc" and hvdc.get("limit_rows") == 0:
            props["window_note"] = (
                "No RTD HVDC limit events recorded in "
                "this archive window; binding evidence is "
                "from IEMOP's monthly reports."
            )
        coords = corridor_routes.get(c["id"], c["coords"])
        props["route"] = "osm-mapped" if c["id"] in corridor_routes else "schematic"
    ...
    # Recorded day profiles, saved comparison cases, and the historical replay.
    from chrono import build_backcast, build_chrono_golden
    from profiles import build_profiles

    merit_hydro = {
        g: dispatch["merit_order"][g]["fuel_avail_mw"].get("hydro")
    ...
    # the derived offer books ship as per-day files the studio's offer
    # mode lazy-fetches (web/data/offers/OFFERD_YYYYMMDD.json)
    import shutil

    offers_src = os.path.join(
    ...
    if os.path.isdir(offers_src):
        ...
        for name in sorted(os.listdir(offers_src)):
            if name.endswith(".json"):
                shutil.copyfile(
                    os.path.join(offers_src, name), os.path.join(offers_out, name)
                )
    ...
    from market_obs import (
        build_admin_dispatch,
        build_advisories,
        build_constrained_on,
        build_corridor_cap_probe,
        build_drivers,
        build_flow_record,
        build_gwap_trigger,
        build_joint_lp_probe,
        build_mot_dispatch_cut,
        build_not_offered,
        build_outlook,
        build_price_setters,
        build_ramp_probe,
        build_reserve_aware,
        build_reserve_prices,
        build_reserve_registration,
        build_reserve_results,
        build_reserve_validation,
        build_security_limits,
        build_settlement_side,
        build_so_instructions,
    ...
    market_ops = {
        ...
        "reserve_aware": build_reserve_aware(
            reserve_validation, reserve_prices, prices
        ),
        ...
        "gwap_trigger": build_gwap_trigger(profiles.get("chrono_golden"), profiles),
    ...
    drivers = build_drivers(prices, profiles, pasa, advisories, reserve_prices)
    ...
    rtdoe5 = (
        json.load(open(r5_path)) if os.path.isfile(r5_path) else {"available": False}
    )
    ...
    expansion = (
        json.load(open(exp_path)) if os.path.isfile(exp_path) else {"available": False}
    )
    ...
    nodal_path = os.path.join(HERE, "..", "data", "derived", "nodal_dcopf.json")
    nodal = (
        json.load(open(nodal_path))
        if os.path.isfile(nodal_path)
    ...
    ls_path = os.path.join(HERE, "..", "data", "derived", "loss_surface.json")
    loss_surface = (
    ...
    # analyst-facing CSV exports, generated from the JSON just written
