#!/usr/bin/env python3
"""Run analyst what-if scenarios through the dispatch calculation
...
def _co2_proxy(s):
    """coal+gas MWh as a dirty-generation proxy (no emissions factor generated)."""
    fm = s["fuel_mwh"]
...
scen(
    "1",
    "+1,000 MW solar, Luzon",
    TYPICAL,
    {"solar_delta_mw": {"luzon": 1000}},
    lambda b, s: (
        "PASS"
        if delta(b, s, "luzon_mid_price") < -0.001
        and abs(delta(b, s, "luzon_peak_price")) < 0.05
        and _co2_proxy(s) < _co2_proxy(b)
        else "CHECK",
...
scen(
    "2a",
    "+300 MW data center, Pampanga (Luzon)",
    TYPICAL,
    {"demand_delta": {"luzon": 300}},
    lambda b, s: (
        "PASS"
        if delta(b, s, "luzon_peak_price") >= -0.001
        and s["leyte_sat_hours"] <= b["leyte_sat_hours"] + 0
        else "CHECK",
...
scen(
    "2b",
    "+1,000 MW data center, Cebu (Visayas)",
    TYPICAL,
    {"demand_delta": {"visayas": 1000}},
    lambda b, s: (
        "PASS"
        if s["leyte_sat_hours"] >= b["leyte_sat_hours"]
        and delta(b, s, "visayas_peak_price") > 0.001
        else "CHECK",
...
scen(
    "3",
    "+50 MW small hydro, Luzon (downstream Angat)",
    TYPICAL,
    {"fuel_avail_delta": {"luzon": {"hydro": 50}}},
    lambda b, s: (
        "PASS"
        if s["fuel_mwh"].get("hydro", 0) >= b["fuel_mwh"].get("hydro", 0)
        and delta(b, s, "luzon_mean_price") <= 0.001
        else "CHECK",
...
scen(
    ...
    lambda b, s: (
        "PASS"
        if delta(b, s, "visayas_mean_price") <= 0.001
        and s["leyte_sat_hours"] <= b["leyte_sat_hours"]
        else "CHECK",
...
scen(
    "5",
    "Malampaya depletes -> imported LNG (gas 4.8->10.3)",
    TYPICAL,
...
scen(
    "6",
    "Trip both Sual units (2x647 MW coal, Luzon)",
    STRESS,
    {"fuel_avail_delta": {"luzon": {"coal": -1294}}},
    lambda b, s: (
        "PASS"
        if delta(b, s, "luzon_peak_price") > 0.001
        or delta(b, s, "luzon_shed_mwh") > 0.1
        else "CHECK",
        f"Luzon peak {delta(b, s, 'luzon_peak_price'):+.3f}, "
...
scen(
    "7",
    "Dry year (hydro x0.344) plus DICT 1.5 GW demand, Luzon",
    STRESS,
...
def main():
    ...
    if "--json" in sys.argv:
        json.dump(
            results,
            open(os.path.join(ROOT, "tmp", "scenario-validation-results.json"), "w"),
            indent=1,
        )
