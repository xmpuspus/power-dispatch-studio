...
Not a production-cost optimiser. This stacks the sourced fleet
(pipeline/fleet_ph.py) by marginal cost against the archive's recorded dispatch,
per grid, per interval, and
reads off the marginal clearing price. It checks that result against the observed
load-weighted average price (LWAP). A competitive cost stack matches normal hours
and under-prices tight hours. The remaining gap is the scarcity and offer premium
a pure cost model cannot see. The output shows that gap instead of inflating fuel
costs to imitate price spikes and spoiling the off-peak match.
...
Outputs generated to web/data/dispatch.json (+ generators.geojson):
  - merit_order: the per-grid supply stack the Simulate panel recalculates
    in the browser
...
def _marg_label(fuel: str | None, price: float) -> str:
    ...
    if fuel == "coal":
        return (
            "coal (committed)"
            if abs(price - COAL_COMMIT_PHP_KWH) < 0.01
            else "coal (marginal)"
        )
    return fuel
...
def _duration_curve(vals: list[float], step: int = 2) -> list[dict]:
    ...
    n = len(s)
    return [
        {"pct": p, "price": round(s[min(n - 1, int(p / 100 * n))], 3)}
...
def build_coupling(intervals: dict, prices: dict) -> dict:
    ...
    price spread the coupled model reproduces and the part it cannot.
    ...
    any tuning of the model inputs.
    """
    ...
    # The map's 5-minute coupled replay used the
    ...
    base_leyte = next(c["limit_mw"] for c in CORRIDORS if c["id"] == "leyte_luzon_hvdc")
    ...
    for (day, ti), iv in intervals.items():
        ...
        res = clear_coupled(
            demand,
            iv["hour"],
            caps={"leyte_luzon_hvdc": leyte_cap, "mvip_hvdc": base_mvip},
        )
    ...
    def explained(model_series: dict, hi: str, lo: str) -> dict:
        ...
        return {
            "observed_php_kwh": s_obs,
            "coupled_model_php_kwh": s_mod,
    ...
    for g in grids:
        ...
        per_grid[g] = {
            ...
            "mae_php_kwh": (
                round(sum(abs(a - b) for a, b in zip(m, o)) / len(o), 3) if o else None
            ),
    ...
    coupling = {
        ...
        "observed_corridor_caps": {
            "leyte_luzon_hvdc": {
                ...
                "capped_share_pct": (
                    round(100 * n_capped / n_coupled, 1) if n_coupled else None
                ),
                "mean_applied_cap_mw": (
                    round(_mean(cap_series), 1) if cap_series else None
                ),
            },
            "note": (
                "The base coupled replay now scales the Leyte-Luzon cap "
...
def _coupling_outage(intervals: dict, prices: dict) -> dict:
    """Labeled scenario: recalculate the streak window with the documented Visayas
    ...
    s_obs = (
        round(_mean(base_obs["visayas"]) - _mean(base_obs["luzon"]), 3) if n else None
    )
    s_mod = round(_mean(out_cpl["visayas"]) - _mean(out_cpl["luzon"]), 3) if n else None
    return {
...
def build_scenario_golden(merit_order: dict) -> dict:
    """Reference input/output pairs computed by the HiGHS LP snapshot engine.

    The browser snapshot engine solves the same single-hour LP through the
    HiGHS wasm build. The TypeScript test rebuilds these pairs
    ...
    and rents below to the rounding. The older independent cost calculation stays
    in this module for analytics and as a separate cost check.
    """
    ...
    cases = [
        {
            "label": "baseline typical evening",
            "demand": dict(base),
            "removed": {},
            "caps": {},
        },
        ...
        },
        {
            ...
            "demand": {**base, "VISAYAS": base["VISAYAS"] + 900},
            "removed": {},
            "caps": {"leyte_luzon_hvdc": c1 + 250},
        },
    ]
    out = []
    for c in cases:
        stacks = {
            g.lower(): stack(g, PEAK_HOUR, (c["removed"] or {}).get(g))
            for g in ("LUZON", "VISAYAS", "MINDANAO")
        }
        demand = {g.lower(): c["demand"][g] for g in ("LUZON", "VISAYAS", "MINDANAO")}
        caps = {
            "leyte": c["caps"].get("leyte_luzon_hvdc", c1),
...
def _coupling_binding_threshold(intervals: dict) -> dict:
    """Forward demand case: at a typical
    ...
    base = {
        g: round(sum(d[g] for d in eve) / len(eve))
        for g in ("LUZON", "VISAYAS", "MINDANAO")
    }
    thr = None
...
def build_reliability_mc(hourly_dem: dict, draws: int = 20000, seed: int = 42) -> dict:
    ...
    def run(loads, base_eve, g):
        return _mc_distribution(
            rng, loads, base_eve, det_named[g], named[g], run_cap, draws
        )
...
def build_storage(hourly_dem: dict, draws: int = 20000, seed: int = 42) -> dict:
    ...
    questions: how much does the existing 634 MW of batteries plus 685 MW of
    pumped hydro shave the peak price after the DICT demand increase, and how
    much does it reduce the loss-of-load probability. Existing storage is
    already in the observed prices,
    so this is a forward scenario with modeled added demand, not a calibration
    ...
    # peak price after the DICT demand increase, with and without storage discharge,
    ...
    p95_eve = (
        eve_luzon[min(len(eve_luzon) - 1, int(0.95 * len(eve_luzon)))]
        if eve_luzon
        else 0
    )
    ...
    return {
        "assets": {
            "luzon": {
                "bess_mw": lz["bess"],
                "pumped_hydro_mw": lz["pumped_hydro"],
                "total_mw": storage_mw,
            },
        },
        ...
        "dict_wave_peak_price": {
            "reference": "95th-percentile evening demand plus the DICT 1.5 GW forecast",
            "demand_mw": dc_demand,
        ...
        "reliability_buyback": {
            "luzon_baseline": {
                ...
                "lolp_with_pct": dist(lz_base_load, av + storage_mw, "LUZON")[
                    "lolp_pct"
                ],
            },
            "luzon_dict_2028": {
...
def build_dispatch() -> dict:
    ...
    hourly: dict[str, dict[int, list]] = {g: {h: [] for h in range(24)} for g in GRIDS}
    ...
    hourly_dem: dict[str, dict[int, list]] = {
        g: {h: [] for h in range(24)} for g in GRIDS
    }
    days_seen: set[str] = set()
    # per-interval simultaneous demand across all three grids, for the coupled model
    intervals: dict[tuple, dict] = {}

    # Check prices only in the market window after WESM resumed. Including an
    # administered ~P6 evening would sit on the modeled coal line and make the
    # result look more accurate. This loop is gated on price
    ...
    for path in rtd:
        ...
        for r in rows_of(path):
            ...
            iv = intervals.setdefault(
                (day, ti), {"hour": hour, "market": is_market, "dem": {}}
            )
            iv["dem"][grid] = load
    ...
    for g in GRIDS:
        ...
        # The price gap (observed minus modeled) flips sign across the day: the
        ...
        # scarcity signal is the evening-peak gap (hours 17-21), where observed
        # exceeds modeled every hour.
        peak_pts = [(mp, op) for h in range(17, 22) for _g, mp, op in hourly[g][h]]
        peak_resid = (
            round(sum(op - mp for mp, op in peak_pts) / len(peak_pts), 3)
            if peak_pts
            else None
        )
        corr = _corr(m, o)
        calibration[g.lower()] = {
            ...
            "correlation": corr,
            "note": (
                "modeled price is flat (demand never leaves the coal block); "
    ...
    # Compare results before and after the unit-commitment layer. The
    ...
    unit_commitment = {
        ...
        "modeled price does not move, and that difference stays in the reported gap.",
        "per_grid": {},
    }
    for g in GRIDS:
    ...
    for g in GRIDS:
        ...
        # of mutating the already-split generated blocks (which would misattribute a
        # coal trip to the wrong tranche). Pipeline stays the source of truth.
        fuel_avail = {
            f: round(avail_mw(g, f, PEAK_HOUR), 1)
            for f in GRID_FUEL_MW[g]
            if avail_mw(g, f, PEAK_HOUR) > 0
        }
        merit_order[g.lower()] = {
            "reference_hour": PEAK_HOUR,
            ...
            "evening_peak_demand_mw": round(
                max(
                    (max(hourly_dem[g][h]) for h in range(18, 22) if hourly_dem[g][h]),
                    default=0.0,
                )
            ),
    ...
    for u in GENERATORS:
        ...
        # peak against the solar-stripped stack had the same timing error the
    ...
    n1.sort(
        key=lambda x: (
            -x["shortfall_at_peak_mw"],
            -x["delta_price_php_kwh"],
            -x["capacity_mw"],
        )
    )
    ...
    # stack understated every margin because it compared different hours. Two
    ...
    dict_anchor = next(
        (a for a in DEMAND_ANCHORS if a.get("owner") == "DICT" and a.get("mw") == 1500),
        None,
    )
    added = 1500
    ...
    # is the measured result; the fixed-evening-availability version counted
    ...
    adequacy["dict_2028"] = {
        "added_mw": added,
        "owner": "DICT",
        "label": "forecast: PH data-center capacity by 2028",
        "date": "2025-10",
        ...
        "reserve_margin_with_dc_pct": round(
            (lz_firm_av - lz_eve_pk - added) / (lz_eve_pk + added) * 100, 1
        )
        if lz_eve_pk
        else None,
    ...
    # hydrology forward axis: a dry (El Nino) / normal / wet multiplier on the generated
