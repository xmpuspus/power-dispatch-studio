#!/usr/bin/env python3
"""Calculate a lowest-cost mix of new generating capacity.

The LT Plan view applies the DOE's own build lists on a horizon slider, no
optimizer. This is a limited optimizer: a lowest-cost new-build capacity
...
It is compared with the DOE project mix in web/data/projects.json. The result
and comparison are written to data/derived/expansion.json for the studio.
...
def optimize(peak_mw: float) -> dict:
    """Lowest-cost new-build capacity mix (MW per technology) that meets demand and
...
def _doe_mix() -> dict:
    ...
    for r in p.get("rows") or []:
        if isinstance(r, dict):
            agg[r.get("fuel") or "unknown"] = agg.get(
                r.get("fuel") or "unknown", 0.0
            ) + (r.get("mw") or 0.0)
    return agg
...
def derive() -> dict:
    ...
    doe = _doe_mix()
    opt_re = sum(
        opt["mix_mw"].get(t, 0) for t in ("solar", "wind", "hydro", "geothermal")
    )
    doe_re = sum(doe.get(t, 0) for t in ("solar", "wind", "hydro", "geothermal"))
    opt_re_share = round(100 * opt_re / (opt["total_mw"] or 1), 1)
    doe_re_share = round(100 * doe_re / (sum(doe.values()) or 1), 1)
    return {
        "available": True,
        "horizon_year": year,
        ...
        "optimized": {
            **opt,
            "mix_share_pct": _shares(opt["mix_mw"]),
            "re_share_pct": opt_re_share,
        },
        "doe_pipeline": {
            "mix_mw": {k: round(v) for k, v in doe.items()},
            "mix_share_pct": _shares(doe),
            "re_share_pct": doe_re_share,
        },
        "verdict": (
            f"The lowest-cost new-build scenario is {opt_re_share}% renewable, "
            f"compared with {doe_re_share}% "
            "in the Department of Energy project list. This compares renewable "
            "shares, not individual plants."
        ),
        "costs_note": (
            "Generic technology costs, NREL ATB 2024 moderate case, 2022 USD "
            "annualized (CRF 0.09) and converted at 56 PhP/USD. Labeled "
            "generic, not PH-specific."
        ),
        "src": "https://atb.nrel.gov/electricity/2024/",
        "disclaimer": (
            "Statistical indicators derived from public data. Patterns may "
...
def main() -> int:
    ...
    if a.derive:
        ...
        print(
            f"expansion: {out['horizon_year']} peak {out['peak_mw']} MW; "
