#!/usr/bin/env python3
"""Generate the LT Plan layer: the DOE's committed and indicative power project
...
The parser keeps the aggregate accurate and refuses to guess names:
  ...
  - per-grid generation totals must reconcile to the DOE's own LVM summary PDF
    or the data build fails loudly;
...
FILES = {
    ("committed", "luzon"): (
        "doe_luzon_committed_2025-12-31.txt",
        "01-luzon-committed-8-pdf",
        "20260330220811",
    ),
    ("committed", "visayas"): (
        "doe_visayas_committed_2025-12-31.txt",
        "02-visayas-committed-9-pdf",
        "20260330221412",
    ),
    ("committed", "mindanao"): (
        "doe_mindanao_committed_2025-12-31.txt",
        "03-mindanao-committed-9-pdf",
        "20260330222524",
    ),
    ("indicative", "luzon"): (
        "doe_luzon_indicative_2025-12-31.txt",
        "06-luzon-indicative-9-pdf",
        "20260330222356",
    ),
    ("indicative", "visayas"): (
        "doe_visayas_indicative_2025-12-31.txt",
        "07-visayas-indicative-9-pdf",
        "20260330222820",
    ),
    ("indicative", "mindanao"): (
        "doe_mindanao_indicative_2025-12-31.txt",
        "08-mindanao-indicative-9-pdf",
        "20260330223944",
    ),
}
SUMMARIES = {
    "committed": (
        "doe_lvm_committed_summary_2025-12-31.txt",
        "04-lvm-committed-summary-9-pdf",
        "20260330215126",
    ),
    "indicative": (
        "doe_lvm_indicative_summary_2025-12-31.txt",
        "09-lvm-indicative-summary-8-pdf",
        "20260330223514",
    ),
}
...
DATE_TOKEN = (
    r"(?:TBD|Completed|[A-Z][a-z]{2}-\d{2}|"
    r"[A-Z][a-z]+ \d{4}|Q[1-4]\s?\d{4}|\d{4})"
)
ROW_RE = re.compile(
    r"([\d,]+\.\d{2,3})\s+(" + DATE_TOKEN + r")"
    r"(?:\s+(" + DATE_TOKEN + r"))?(?:\s|$)"
)
    ...
    with open(path, encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            for rm in ROW_RE.finditer(line):
                ...
                cur["rows"].append(
                    {"mw": mw, "target": op, "target_year": _year_of(op)}
                )
    out_sections = []
    rows = []
    for s in sections:
        ...
        if reconciled:
            for r in s["rows"]:
                rows.append({"grid": grid, "status": status, "fuel": s["fuel"], **r})
    gen_total = round(
        sum(
            s["subtotal_mw"] for s in sections if SECTION_FUEL[s["header"]] != "storage"
        ),
        2,
    )
    ess_total = round(
...
def _summary_totals(status: str) -> list[float]:
    ...
    totals = []
    with open(os.path.join(DOE_DIR, fname), encoding="utf-8", errors="replace") as f:
        for line in f:
...
SRC_TDP_MAR = (
    "https://www.ngcp.ph/Attachment-Uploads/"
...
TDP_CORRIDORS = [
    {
        "name": "Luzon-Visayas HVDC bipolar upgrade",
        "iface": "leyte_luzon_hvdc",
        "adds_mw": 440,
        "target": "Stage 1 Mar 2028, Stage 2 Dec 2030",
        "target_year": 2028,
        "cost_mphp": 22463,
    ...
    },
    {
        ...
        "iface": "mvip_hvdc",
        "adds_mw": 450,
        "target": "2025-2030 window, no month stated",
        "target_year": 2030,
        "cost_mphp": None,
    ...
    {
        "name": "Batangas-Mindoro interconnection (BMIBP) Stage 1",
        "iface": None,
        "adds_mw": None,
        "target": "TDP says Sep 2027; Mar 2026 press says Jan "
    ...
    {
        "name": "Cebu-Leyte 230 kV lines 3 and 4",
        "iface": None,
        "adds_mw": None,
        "target": "Stage 1 Dec 2028, Stage 2 Dec 2031",
        "target_year": 2028,
        "cost_mphp": None,
    ...
    {
        "name": "Northern Luzon 500 kV extension",
        "iface": None,
        "adds_mw": None,
        "target": "Bolo-Balaoan Oct 2030; Balaoan-Laoag Aug 2031",
        "target_year": 2030,
        "cost_mphp": 40195,
    ...
    {
        "name": "Bataan-Cavite 500 kV line",
        "iface": None,
        "adds_mw": None,
        "target": "feasibility to be assessed; windowed 2031-2040",
        "target_year": None,
        "cost_mphp": None,
...
def build_projects() -> dict:
    if not all(os.path.exists(os.path.join(DOE_DIR, f[0])) for f in FILES.values()):
        return {
            "available": False,
    ...
    for status in ("committed", "indicative"):
        ...
        if len(summary) < 3:
            raise SystemExit(
                f"projects: {status} summary parse found "
        ...
        for i, grid in enumerate(grids):
            ...
            if abs(parsed["gen_total_mw"] - summary[i]) > 0.02:
                raise SystemExit(
                    ...
                    f"{summary[i]} MW; refusing to generate a list with mismatched totals"
                )
            sections.extend(parsed["sections"])
            rows.extend(parsed["rows"])
            editions[status][grid] = {
                "src": parsed["src"],
                "original_url": parsed["original_url"],
            }
            totals[status][grid] = {
                "gen_mw": parsed["gen_total_mw"],
    ...
    return {
        ...
        "ess_note": (
            "Energy storage projects are tracked separately from "
...
if __name__ == "__main__":
    ...
    out = build_projects()
    slim = {
        k: (f"{len(v)} rows" if k == "rows" else v)
