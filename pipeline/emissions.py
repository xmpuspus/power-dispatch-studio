#!/usr/bin/env python3
"""Generate CO2 emission factors per generation technology, with sources.
...
The DOE's published National Grid Emission Factor is generated alongside as an
...
SRC_EMB = (
    "https://emb.gov.ph/wp-content/uploads/2019/11/"
...
def build_emissions() -> dict:
    factor_map = {
        f["fuel"]: f["tco2_per_mwh"] for f in FACTORS if f["tco2_per_mwh"] is not None
    }
    return {
        "available": True,
        ...
        "ngef": {
            ...
            "src": SRC_NGEF,
            "note": (
                "The DOE's published National Grid Emission Factor, the "
                "grid-average across all technologies. A cross-check "
                "anchor for the per-fuel math, not an input to it."
            ),
        },
        "note": (
            "Per-technology operational factors: IPCC 2006 fuel defaults "
