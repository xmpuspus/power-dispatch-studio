#!/usr/bin/env python3
"""Test a joint energy-and-reserve calculation against official reserve prices.

The offer-only reserve replay prices below the official regional reserve price
(RSVPR). A per-resource linear program using the raw energy and reserve offers
does not close the difference. During scarce hours, the market schedules less
reserve than the requirement and applies a scarcity price that is not present
in the public offers.
...
centavo. Where reserve is short, the official price sits above the entire
...
where feasible, below the official scarcity price.
...
def _stamp(date: str, hour: int) -> str:
    return (datetime.strptime(date, "%Y-%m-%d") + timedelta(hours=hour + 1)).strftime(
        "%Y%m%d%H%M"
    )
...
def _official_rsvpr(date, hour, grid):
    iv = _interval(date, hour)
    for p in glob.glob(os.path.join(RAW, "RSVPR", f"*{date.replace('-', '')}*.csv")):
        for r in csv.DictReader(open(p, encoding="utf-8", errors="replace")):
            if (
                r.get("TIME_INTERVAL") == iv
                and r.get("REGION_NAME") == grid
...
def _joint_free_reserve_price(date, hour, grid):
    ...
    iv = _interval(date, hour)
    oe = [
        r for r in oe if r.get("TIME_INTERVAL") == iv and r.get("REGION_NAME") == grid
    ]
    orr = [
        r
        for r in orr
        if r.get("TIME_INTERVAL") == iv
        and r.get("REGION_NAME") == grid
        and r.get("COMMODITY_TYPE") == "Dr"
    ]
...
def _sample(date, hour, grid) -> dict:
    ...
    return {
        ...
        "hour": hour,
        "grid": GRID_NAME[grid],
        "mkt_reqt_mw": round(reqt, 1),
        "scheduled_mw": round(sched, 1),
        ...
        "joint_lp_free_redispatch_php_kwh": _joint_free_reserve_price(date, hour, grid),
    }


def derive() -> dict:
    rows = [d for (date, hour, grid) in SAMPLE if (d := _sample(date, hour, grid))]
    met = [r for r in rows if (r["met_pct"] or 0) >= 99]
    short = [r for r in rows if (r["met_pct"] or 0) < 99]
    max_met_gap = max((abs(r["scarcity_uplift_php_kwh"]) for r in met), default=0.0)
    max_short_uplift = max((r["scarcity_uplift_php_kwh"] for r in short), default=0.0)
    return {
        "available": True,
        "samples": rows,
        "n_samples": len(rows),
        ...
        "note": (
            "The sampled per-resource joint energy-and-reserve LP uses "
            "the raw RTDOE and RTDOR offers but does not reproduce the "
            "official reserve price (RSVPR). On scarce hours, the market "
            "schedules less reserve than the requirement and applies an "
            "administered scarcity value that is not in the offers. On "
            "requirement-met hours the marginal "
            ...
            "official price sits above every submitted offer, an "
            "administered reserve-scarcity value the public offers cannot "
            "explain; a free joint LP that forces the full requirement "
            "is infeasible on exactly those hours. The price difference "
            "therefore reflects administered reserve scarcity that cannot "
            "be recovered from the public offers. The model withholds the "
            "published requirement and shows the official price beside its "
            "result. Fixing the "
...
if __name__ == "__main__":
    ...
    if a.derive:
        ...
        print(
            f"wrote {OUT}: {out['n_requirement_met']} met, "
        ...
        for r in out["samples"]:
            print(
                f"  {r['date']} h{r['hour']} {r['grid']}: met {r['met_pct']}% "
