...
def main():
    ...
    for r in C["league"]:
        ...
        if osm not in by_name:
            continue
        seen.add(st)
        ranked.append({**r, "osm": osm, "xy": by_name[osm],
                       "hot": r["equipment"] in CORRIDOR})
    ...
    if all(s in STATION_OSM and STATION_OSM[s] in by_name for s in CORRIDOR_ARC):
        arc = bezier(by_name[STATION_OSM[CORRIDOR_ARC[0]]],
                     by_name[STATION_OSM[CORRIDOR_ARC[1]]])
    ...
    for f in lines:
        g = f["geometry"]
        for s in ([g["coordinates"]] if g["type"] == "LineString"
                  else g["coordinates"]):
            for p in s:
                xs.append(p[0])
                ys.append(p[1])
    ...
    for fi in range(n_frames):
        ...
        gs = fig.add_gridspec(1, 2, width_ratios=[0.70, 1.0], wspace=0.03,
                              left=0.012, right=0.988, top=0.855, bottom=0.115)
        ...
        for f in lines:
            ...
            for s in segs:
                ...
                if lw > 1.0:                       # a soft glow under the backbone
                    ax.plot(sx, sy, color=col, lw=lw * 3.2, alpha=0.10,
                            zorder=1, solid_capstyle="round")
                ax.plot(sx, sy, color=col, lw=lw, alpha=al, zorder=2,
                        solid_capstyle="round")

        # --- the corridor arc, drawn once the two end stations are lit --------
        lit = min(fi + 1, placed)
        ends_lit = sum(1 for r in ranked[:lit]
                       if r["station"] in CORRIDOR_ARC)
        if arc and ends_lit == 2:
            ...
            for w, a in ((7.0, 0.10), (4.0, 0.18), (1.9, 0.95)):
                ax.add_patch(PathPatch(pth, fill=False, edgecolor=HOT, lw=w,
                                       alpha=a, zorder=4, capstyle="round"))
        ...
        for i, r in enumerate(ranked):
            ...
            for m, a in ((6.0, 0.045), (3.4, 0.085), (1.9, 0.16)):
                ax.scatter([x], [y], s=base * m * pop, color=col, alpha=a,
                           linewidths=0, zorder=5)
            ax.scatter([x], [y], s=base * pop, color=col, alpha=0.92,
                       linewidths=0, zorder=6)
            # the real-time core: the smaller count, the one settlement sees
            rt = base * (r["rtd_days"] / max(1, r["days"]))
            ax.scatter([x], [y], s=max(4.0, rt), color="#ffffff", alpha=0.85,
        ...
        if arc and ends_lit == 2:
            ...
            for f in lines:
                ...
                for s in ([g["coordinates"]] if g["type"] == "LineString"
                    ...
                    col, lw, al = VOLT.get(f["properties"].get("kind"), VOLT_DEF)
                    ix.plot([p[0] for p in s], [p[1] for p in s], color=col,
                            lw=lw * 1.5, alpha=al, zorder=2)
            k = max(2, int(len(arc) * min(1.0, max(0.25, grow))))
            pth = MPath(arc[:k])
            for w, al in ((9.0, 0.10), (5.0, 0.20), (2.4, 0.95)):
                ix.add_patch(PathPatch(pth, fill=False, edgecolor=HOT, lw=w,
                                       alpha=al, zorder=4, capstyle="round"))
            ...
            for pt, lbl, dy, va, ha, dx in (
                ...
                for m, al in ((5.0, 0.10), (2.6, 0.20)):
                    ix.scatter([pt[0]], [pt[1]], s=70 * m, color=HOT, alpha=al,
                               linewidths=0, zorder=5)
                ix.scatter([pt[0]], [pt[1]], s=52, color=HOT, zorder=6,
                           linewidths=0)
                ix.text(pt[0] + dx, pt[1] + dy, lbl, fontsize=6.2, color=TEXT,
            ...
            ix.text(0.03, 0.94, f"the corridor, {gc_km(a, b):.0f} km across",
                    transform=ix.transAxes,
                    fontsize=6.6, color=MUTE, va="top")

        # --- the ranking the bars gave for free -------------------------------
        bx.set_xlim(0, 1)
        ...
        for i, r in enumerate(ranked):
            ...
            col = HOT if r["hot"] else COOL
            w = 0.40 * (r["days"] / top)
            bx.barh(i, w if on else 0, left=0.44, height=0.52,
                    color=col, alpha=0.92 if on else 0.0, zorder=3)
            bx.text(0.42, i, f'{r["equipment"]}', va="center", ha="right",
                    fontsize=8.4, color=TEXT if on else "#39424f",
                    family="monospace", zorder=4)
            if on:
                bx.text(0.44 + w + 0.012, i, f'{r["days"]}', va="center",
                        fontsize=8.2, color=col, zorder=4)
        bx.text(0.44, -0.62, "days at a binding limit", fontsize=7.6,
                color=MUTE, va="bottom")

        # --- titles -----------------------------------------------------------
        fig.text(0.012, 0.972,
                 "The grid names its own choke point, and it sits between two islands",
                 fontsize=14.6, color=TEXT, weight="bold", va="top")
        ...
        fig.savefig(os.path.join(FRAMES, f"f{fi:03d}.png"), dpi=104,
                    facecolor=BG)
        plt.close(fig)
    ...
    subprocess.run(["ffmpeg", "-y", "-i", os.path.join(FRAMES, "f%03d.png"),
                    "-vf", vf + ",palettegen=stats_mode=diff:max_colors=128", pal],
    ...
    print(f"wrote {OUT} ({os.path.getsize(OUT) // 1024} KB), {placed} stations placed, "
