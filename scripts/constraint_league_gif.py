#!/usr/bin/env python3
"""Animate equipment ranked by days at a binding limit.

The Leyte-Cebu corridor reaches a binding limit most often in the date range.
...
def main():
    ...
    for fi, g in enumerate(frames):
        ...
        ax.set_title(
            "IEMOP records name the equipment that reaches its limit most often",
            fontsize=14,
            color=vz.NAVY,
            loc="left",
        )
        ...
        vz.caption(
            ...
            "Source: IEMOP RTDCV and DAPCV, archived.",
            y=-0.03,
        )
        fig.tight_layout()
        fig.savefig(
            os.path.join(FRAMES, f"f{fi:03d}.png"),
            dpi=110,
            bbox_inches="tight",
            facecolor="white",
        )
