...
(docs/hero.png) from the generated data, so the preview image follows the data build. The
...
def render(path, w_in, h_in, dpi):
    ...
    dates = P["dates"]
    resume_i = next(
        (i for i, d in enumerate(dates) if d >= P.get("resumed", "2026-05-01")), 0
    )
    ...
    ax.set_xticks([])
    ax.set_title(
        f"{C['distinct_equipment']} named pieces of equipment hit a "
