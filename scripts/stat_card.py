...
announced data-center demand as a share of the whole system's spare margin, with both
figures labeled by owner and source. Numbers come from the data build (market_anchors,
...
WHITE = "#f4f7fa"
MUTE = "#9db0c4"
CORAL = vz.CORAL  # announced demand
...
def main():
    ...
    T(
        0.93,
        0.945,
        "the Philippine grid, from the operator's own files",
        12.5,
        MUTE,
        ha="right",
    )
    ...
    T(0.075, 0.63, "of the grid's entire spare margin is the size", 27, WHITE, "bold")
    T(0.075, 0.585, "of the announced data-center demand.", 27, WHITE, "bold")
    ...
    ax.add_patch(
        plt.Rectangle(
            (x0, y_marg - h / 2), full, h, transform=ax.transAxes, color=GREEN, zorder=2
        )
    )
    T(x0, y_wave + 0.048, "ANNOUNCED DEMAND", 13, MUTE, "bold")
    ...
    T(0.075, 0.235, "Today's data centers are small and did not cause the", 16, MUTE)
    T(
        0.075,
        0.198,
        "recent red alerts. The question is where the announced demand",
        16,
        MUTE,
    )
    T(0.075, 0.161, "landing on a grid whose margin is already this thin.", 16, MUTE)

    ax.plot(
        [0.075, 0.925], [0.105, 0.105], transform=ax.transAxes, color="#24425f", lw=1.2
    )
    T(0.075, 0.072, "power-dispatch-studio.vercel.app", 18, WHITE, "bold")
    T(
        0.075,
        0.037,
        "Free. Runs in your browser. Sources and assumptions are labeled.",
        13.5,
        MUTE,
    )
    ...
    plt.close(fig)
    print(
        "wrote", out, f"({os.path.getsize(out) // 1024} KB)  {pct}% = {wave} / {margin}"
    )
...
def _choke():
    ...
    from the operator's own 5-minute congestion files. Numbers from the data build."""
    cg = json.load(open(os.path.join(WEB, "congestion.json")))
    mo = json.load(open(os.path.join(WEB, "market_ops.json")))
    days = cg["days_covered"]
    top = max(
        (r for r in cg["league"] if "5DAAN_4TAB2" in r["equipment"]),
    ...
    T(0.075, 0.50, "The grid names its own weak spot. The market", 17, MUTE)
    T(0.075, 0.463, "operator publishes the exact line at its limit,", 17, MUTE)
    T(0.075, 0.426, "every 5 minutes. This is the receipt.", 17, MUTE)
    T(0.075, 0.33, f"{share}%", 60, CORAL, "bold")
    T(0.075, 0.245, "of every line-limitation instruction the operator", 16, MUTE)
    T(0.075, 0.208, "wrote down names this one corridor. It sits on the", 16, MUTE)
    T(0.075, 0.171, "path the added data-center demand would have to cross.", 16, MUTE)
    ax.plot(
        [0.075, 0.925], [0.105, 0.105], transform=ax.transAxes, color="#24425f", lw=1.2
    )
    T(0.075, 0.072, "power-dispatch-studio.vercel.app", 18, WHITE, "bold")
    T(
        0.075,
        0.037,
        "IEMOP congestion + dispatch-instruction files, archived daily.",
        13.5,
        MUTE,
    )
...
def _wide(margin, wave, pct):
    ...
    T(0.05, 0.31, "question is where the announced demand lands", 15.5, MUTE)
    ...
    T(rx, 0.74, "data-center demand.", 25, WHITE, "bold")
    ...
    ax.add_patch(
        plt.Rectangle(
            (rx, y_marg - h / 2), full, h, transform=ax.transAxes, color=GREEN, zorder=2
        )
    )
    T(rx, y_wave + 0.055, "ANNOUNCED DEMAND", 13, MUTE, "bold")
