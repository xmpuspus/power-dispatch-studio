...
The rules this encodes, all of them borrowed from the Information is Beautiful
method and then checked against the Tufte veto:
  ...
  - The payoff number sits large in the accent, parked in empty space, away
    from the series.
  - Direct labels beside marks, no legend unless a label cannot fit.
  - Every number traces to the source line typeset into the card.

Animation is a reveal, never decoration: `ease` grows a series in, so a reader
watches the quantity arrive rather than meeting it finished.

    import cardstyle as cs
    fig, ax = cs.card(figsize=(8.6, 5.0), field="dusk")
    cs.title(fig, "The claim", "the measure, the unit, the date")
    cs.payoff(ax, 0.72, 0.30, "18%", "of the margin")
...
def card(figsize=(8.6, 5.0), field="dusk", rect=(0.075, 0.175, 0.90, 0.60)):
    ...
    `rect` is explicit because the title, the payoff and the source line are
...
def title(fig, claim, sub=None, pop=None):
    ...
    fig.text(0.075, 0.945, claim, fontsize=16.5, color=TEXT, weight="bold",
             va="top", zorder=5)
    if pop:
        fig.text(0.075, 0.945, claim.split(pop)[0], fontsize=16.5,
                 color="none", va="top", zorder=5)
    if sub:
        fig.text(0.075, 0.868, sub, fontsize=9.4, color=MUTE, va="top", zorder=5)


def payoff(fig, x, y, big, small=None, color=CORAL, size=34):
    """The number the reader carries away, parked in empty space."""
    fig.text(x, y, big, fontsize=size, color=color, weight="bold",
             va="center", ha="left", zorder=6)
    if small:
        fig.text(x, y - 0.062, small, fontsize=9.2, color=MUTE,
                 va="center", ha="left", zorder=6)
...
def chip(ax, x, y, s, color=TEXT, size=9.0, ha="center", va="center", pad=0.28):
    ...
    ax.text(x, y, s, fontsize=size, color=color, ha=ha, va=va, zorder=9,
...
def glow(ax, xs, ys, color, lw=2.0, zorder=5, passes=((5.2, 0.09), (2.6, 0.16))):
    ...
    for m, a in passes:
        ax.plot(xs, ys, color=color, lw=lw * m, alpha=a, zorder=zorder,
                solid_capstyle="round")
    ax.plot(xs, ys, color=color, lw=lw, zorder=zorder + 1,
            solid_capstyle="round")


def dot(ax, x, y, color, size=70, zorder=7):
    for m, a in ((6.0, 0.06), (3.2, 0.12), (1.8, 0.20)):
        ax.scatter([x], [y], s=size * m, color=color, alpha=a, linewidths=0,
                   zorder=zorder)
...
def rounded(fig, ax, color=PANEL, alpha=0.55):
    ...
    fig.patches.append(FancyBboxPatch(
        (b.x0 - 0.012, b.y0 - 0.03), b.width + 0.024, b.height + 0.06,
        boxstyle="round,pad=0,rounding_size=0.02", transform=fig.transFigure,
...
def save_gif(fdir, out, fps=12, width=880, colors=128):
    ...
    subprocess.run(["ffmpeg", "-y", "-i", f"{fdir}/f%03d.png", "-vf",
                    f"{vf},palettegen=stats_mode=diff:max_colors={colors}", pal],
                   check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    subprocess.run(["ffmpeg", "-y", "-framerate", str(fps),
                    "-i", f"{fdir}/f%03d.png", "-i", pal, "-lavfi",
