...
BCDA says Pax Silica at New Clark City will build its own power on site rather
than lean on the grid for the whole 3,000 MW. It has not turned grid supply down.
This tests the on-site plan against the network. The question is
...
SEGMENTS = [
    ("from the grid, over the one route that feeds it", vz.STEEL),
...
def main():
    ...
    vz.caption(
        fig,
        "The 3,000 MW is BCDA's figure at full development, which it puts 10 to "
        "15 years after construction starts in 2028. It is held flat here from "
        "day one. The solar is the 500 MW ACWA project on a cloudless day. NGCP "
        "does not publish what the Concepcion to Clark lines are rated to carry, "
        "so the model rates each 230 kV circuit at a class default of 400 MW, "
        ...
        "to the rest of the grid. Losing that route means losing both of its "
        "circuits at once, and it takes Pax Silica to zero. Whether the real grid "
        "has a second way in is not something a public map can answer, and no "
        "line outage is modelled here. The 2,500 MW station and the 600 MW unit "
        "are both sizes chosen for this comparison, not announced projects; at "
        "417 MW, the largest unit in the outage record, the last row would leave "
        "148 MW rather than 331. In the third row Pax Silica needs only 500 MW "
        "from the grid, so that is all it takes. In the fourth it needs 1,100 and "
