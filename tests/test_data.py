#!/usr/bin/env python3
"""Data-integrity checks on generated files. Pins checked constants and
...
# --- raw-archive convention pins (upstream of every generated number) ---------------
# These guard two conventions that are invisible in the generated output but corrupt
    ...
    if _lw:
        ...
        for _r in csv.DictReader(open(_lw[len(_lw) // 2])):
            ...
            if _h is not None:
                _bins[_h] += 1
        check(
            "every hour bins exactly 12 five-minute intervals "
            "(interval-ending convention)",
            sum(_bins.values()) == 288 and all(_bins[h] == 12 for h in range(24)),
        )
        ...
        for _p in _gw[:5]:
            for _r in csv.DictReader(open(_p)):
                ...
                if _rn:
                    _seen.add(_rn)
        check(
            "no unmapped GWAPF region label appears in the archive",
            _seen <= _KNOWN_GWAPF,
        )
except ImportError:
    pass
...
check(
    "every chokepoint has evidence + src",
    all(
...
check(
    "every corridor rides the OSM-routed geometry",
    all(
...
check(
    "Leyte-Luzon HVDC 440 MW nameplate / 250 MW limit",
...
check(
    "every DC site has src + city precision",
    all(
...
check(
    "meta named_dc_mw_total matches", math.isclose(meta["named_dc_mw_total"], named_mw)
)
statuses = {f["properties"]["status"] for f in dc["features"]}
check(
    "dc statuses valid",
    statuses <= {"operational", "under_construction", "planned", "announced"},
)
...
check(
    "league rows have equipment+station+day count",
    all(
...
# ~96% of real-time rows (the dispatch holds flow AT the limit, not over it),
# so a league
...
# drops below 100 on a congestion row, so it is the reliable signal.
check(
    "every league row carries a percent-of-limit severity at or above 100 "
...
check(
    "chokepoints that never overloaded still show they were fully bound "
...
# method page promises. This check keeps the exporter and generated file in sync.
...
if os.path.isfile(_csv):
    ...
    check(
        "the committed congestion CSV export carries the percent-of-limit "
        "severity column (exporter and generated file in sync)",
...
check(
    "league sorted by days descending", league_days == sorted(league_days, reverse=True)
)
check(
    "league keeps RTD and DAP counts separate",
    all("rtd_intervals" in e and "dap_rows" in e for e in cong["league"]),
)
...
check(
    "Leyte-Cebu receipts name LEYTE_TO_CEBU",
    any(m["name"] == "LEYTE_TO_CEBU" for m in lc.get("matched_equipment", [])),
)
lc_feat = next(
    f["properties"]
    for f in ck["features"]
...
check(
    "Leyte-Cebu line carries its receipts on the geojson",
...
check(
    "every finding complete + has a map focus",
    all(
        all(f.get(k) for k in ("id", "tag", "title", "stat", "blurb", "source"))
        and f.get("focus", {}).get("center")
        and f["focus"].get("mode")
        for f in fnd
    ),
)
check(
    "a finding names the LEYTE_TO_CEBU receipt",
    any("LEYTE_TO_CEBU" in f["stat"] or "LEYTE_TO_CEBU" in f["blurb"] for f in fnd),
)

# reliability from RTDSUM
check(
    "reliability has 3 grids",
    set(rel.get("series", {})) == {"luzon", "visayas", "mindanao"},
)
check("reliability covers 80+ days", len(rel.get("dates", [])) >= 80)
check(
    "curtailment observed in window (May 2026 red alerts)",
    any(rel["totals"][g]["curtailment_days"] > 0 for g in rel["totals"]),
)

# prices from LWAPF (PhP/kWh after /1000)
check(
    "price series 3 grids",
    set(prices.get("series", {})) == {"luzon", "visayas", "mindanao"},
)
check("price days 70+", len(prices.get("dates", [])) >= 70)
sane = [v for g in prices["series"].values() for v in g if v is not None]
check("prices in sane PhP/kWh band (0-33)", sane and all(0 <= v <= 33 for v in sane))
...
check(
    "Luzon load axis is grid-scale (thousands of MW, not bid-in load)",
    lz_curve and lz_curve[-1]["gen_mw"] > 8000,
)
check(
    "price rises with load (the shape)",
    lz_curve and lz_curve[-1]["mean_price"] > lz_curve[0]["mean_price"],
)
rep = pl.get("representative_day", {})
check(
    "representative day has a full Luzon interval series",
...
check(
    "price regimes split administered vs market", set(reg) == {"administered", "market"}
)
check(
    "administered window prices are near-flat across grids",
    (reg.get("administered", {}).get("max_spread") or 99) < 0.2,
)
check(
    "market window shows real regional spread",
    (reg.get("market", {}).get("mean_spread") or 0) > 1,
)
check("prices carry an as_of date", bool(prices.get("as_of")))

# HVDC: no RTD limit events landed in-window; the generated data records that
# absence rather than inventing a series.
hvdc_j = load("hvdc.json")
check("hvdc data build records the in-window limit-row count", "limit_rows" in hvdc_j)

# answers: the three questions, fully assembled and sourced
for q in ("q1", "q2", "q3"):
    check(
        f"answers.{q} complete",
        all(
            ans.get(q, {}).get(k) for k in ("title", "verdict", "stat", "blurb", "src")
        ),
    )
check(
    "q1 cites the margin figure",
...
check(
    ...
    all(
        str(anchors[k]) in ans["q3"]["stat"]
        for k in ("wesm_may2026_luzon", "wesm_may2026_visayas", "wesm_may2026_mindanao")
    ),
)

# anchors pinned to their sources
check(
    "May 2026 system avg 7.79 +38.5%",
...
check(
    "Visayas streak is the dated 52-day fact",
    anchors.get("visayas_yellow_streak_days") == 52
    and anchors.get("visayas_yellow_streak_to") == "2026-07-01",
)
check(
    "Visayas streak carries both first-party sources",
    anchors.get("src_visayas_streak_end") and anchors.get("src_visayas_jul1"),
)
check(
    "every demand anchor has src + owner + kind",
    all(a.get("src") and a.get("owner") and a.get("kind") for a in demand),
)
check(
    "forecasts labeled (no bare-fact 1.5 GW)",
    all(
        a["kind"] in ("forecast", "commitment", "pipeline", "contested") for a in demand
    ),
)
...
check(
    "every generator has src + fuel + capacity + city precision + marginal cost",
    all(
        p.get("src")
        and p.get("fuel")
        and p.get("capacity_mw")
...
check(
    "named movers include Ilijan, Sual, Dinginin, TVI, PEDC",
    {"Ilijan", "Sual", "GNPower Dinginin", "Therma Visayas (TVI)", "PEDC"} <= gnames,
)

# fleet reconciliation: the per-grid fuel split is a MODEL allocation, but its
# columns must sum exactly to the sourced national fuel totals.
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "pipeline"))
...
for g in fl.GRIDS:
    ...
    check(
        f"fleet {g} row {row} MW stays at or under the published "
        f"{fl.GRID_TOTAL_MW[g]} MW grid total",
        row <= fl.GRID_TOTAL_MW[g],
    )
check(
    "all natural gas is on Luzon (Batangas/Malampaya, sourced)",
    fl.GRID_FUEL_MW["VISAYAS"]["natural_gas"] == 0
    and fl.GRID_FUEL_MW["MINDANAO"]["natural_gas"] == 0,
)
check(
    "coal marginal cost is the sourced ERC administered P6.00/kWh",
    math.isclose(fl.FUEL_COST_PHP_KWH["coal"], 6.00),
)
check(
    "Malampaya gas cost sourced at P4.80/kWh below coal",
    math.isclose(fl.FUEL_COST_PHP_KWH["natural_gas"], 4.80)
    and fl.FUEL_COST_PHP_KWH["natural_gas"] < fl.FUEL_COST_PHP_KWH["coal"],
)

check(
    "dispatch model available + labeled simplified",
...
check(
    "calibration is market-window only (WESM resumed 2026-05-01)",
    cw.get("regime") == "market-only"
    and cw.get("from") == anchors["wesm_resumed"]
    and cw.get("days", 0) >= 40,
)
cal = disp["calibration"]
check("calibration covers 3 grids", set(cal) == {"luzon", "visayas", "mindanao"})
check(
    "model under-predicts on average (cost stack < observed LWAP)",
    all(cal[g]["modeled_mean_php_kwh"] < cal[g]["observed_mean_php_kwh"] for g in cal),
)
check(
    "recorded evening price exceeds the cost model on every grid",
    all((cal[g]["evening_peak_residual_php_kwh"] or 0) > 0 for g in cal),
)
check(
    "Visayas residual is the largest (the 52-day constrained grid)",
    cal["visayas"]["evening_peak_residual_php_kwh"]
    > cal["luzon"]["evening_peak_residual_php_kwh"],
)
check(
    "merit_order stack generated for 3 grids with cost-ordered blocks",
    all(
...
check(
    "N-1 table covers all 11 named units",
    len(disp["n1"]) == 11 and all("shortfall_at_peak_mw" in n for n in disp["n1"]),
)
check(
    "N-1 orders by peak shortfall; the biggest units shed the most",
    disp["n1"][0]["shortfall_at_peak_mw"] >= disp["n1"][-1]["shortfall_at_peak_mw"],
)
adq = disp["adequacy"]["dict_2028"]
check(
    "DICT 1.5 GW scenario is a labeled DICT forecast with src",
    adq["owner"] == "DICT" and adq["added_mw"] == 1500 and adq.get("src"),
)
check(
    "adding the DICT demand forecast reduces the Luzon reserve margin",
    adq["reserve_margin_with_dc_pct"] < adq["reserve_margin_now_pct"],
)
check(
    "the DICT demand forecast leaves no interval short against the hour-matched stack",
    adq["shortfall_intervals_with_dc"] == 0 and adq["eue_mwh_with_dc"] == 0,
)
check(
    "the solar-adjusted tightest interval with DICT demand remains positive",
    adq["tight_reserve_margin_with_dc_pct"] > 0,
)
for _g in ("luzon", "visayas", "mindanao"):
    _a = disp["adequacy"][_g]
    _re = round(
        (_a["avail_at_peak_mw"] - _a["evening_peak_demand_mw"])
        / _a["evening_peak_demand_mw"]
        * 100,
        1,
    )
    check(
        f"{_g} reserve margin uses evening available supply and evening peak",
        abs(_re - _a["reserve_margin_pct"]) < 0.5
        and _a["evening_peak_demand_mw"] < _a["gross_peak_mw"],
    )

# Inter-island dispatch must satisfy the Karush-Kuhn-Tucker conditions on a
# radial path. Across an unsaturated corridor, adjacent grids clear at the same
# price within the wheeling dead band. Across a saturated corridor, the exporter
# is cheaper and the congestion rent equals the price gap.
...
for dem, rem in _KKT_CASES:
    ...
    best = min(
        cd.system_cost(dem, 19, x1, x2, removed=rem)
    ...
    for c in r["corridors"]:
        ...
        if c["saturated"]:
            ...
            rent_ok = False
check("coupled solver lands on the min-cost flow (optimality vs brute force)", opt_ok)
check("congestion rent only on a saturated corridor, and equals the price gap", rent_ok)
...
check(
    "no phantom corridor flow when grids tie on coal",
    abs(tie["flow_lv_mw"]) < 1
    and abs(tie["flow_vm_mw"]) < 1
...
check(
    "no VOLL adder: an oil-margin grid caps at the oil cost P12",
    oiled["grids"]["visayas"]["price"] <= fl.FUEL_COST_PHP_KWH["oil"] + 1e-6,
)

cp = disp["coupling"]
check(
    "coupling section generated + labeled simplified",
    "simplified merit-order" in cp.get("model", "")
    and cp.get("n_coupled_intervals", 0) > 1000,
)
check(
    "coupling couples only all-three-grid market intervals",
    cp["calibration_window"]["regime"] == "market-only",
)
sd = cp["spread_decomposition"]
# the measured finding: on baseline demand the corridor almost never binds, so the
# coupled model reproduces almost none of the observed Visayas-Luzon spread
check(
    "observed Visayas-Luzon spread is real (>3 PhP/kWh)",
    sd["visayas_vs_luzon"]["observed_php_kwh"] > 3,
)
check(
    "baseline coupling explains almost none of the V-L spread (the finding: "
...
check(
    "Leyte-Luzon rarely binds on baseline demand (<5% of intervals)",
...
# the whole generated dataset, not just the synthetic solver cases above: the
# earlier bug lived in the generated means, where no test was looking.
_neg_rents = []
...
for _name in sorted(os.listdir(WEB)):
    ...
    try:
        ...
        continue
check(
    "no negative congestion rent; blocked corridors earn nothing"
    f"{'' if not _neg_rents else ': ' + '; '.join(_neg_rents[:4])}",
    not _neg_rents,
)
# The 5-minute coupled replay scales the
# corridor by the observed hourly availability (MPI), not a flat 250 MW
_occ = cp["observed_corridor_caps"]["leyte_luzon_hvdc"]
check(
    "the coupled replay scales the corridor by the observed hourly "
...
check(
    "MVIP cap is labeled nameplate-as-operating-limit (distinct from Leyte's "
    "sourced 250)",
    leyte_stat["limit_kind"] == "sourced_operating_limit"
...
check(
    "outage scenario is a labeled, sourced Visayas outage",
    osc.get("src") and osc["outage_mw"] > 500 and osc["n_intervals"] > 1000,
)
check(
    "under the documented outage the Leyte corridor binds and prices a rent",
    osc["leyte_luzon_saturated_pct"] > 10 and osc["leyte_luzon_mean_rent_php_kwh"] > 1,
)
check(
    "outage scenario explains materially more of the spread than baseline",
    osc["explained_fraction"] > 0.1 > abs(sd["visayas_vs_luzon"]["explained_fraction"]),
)
bt = cp["dc_binding_threshold"]
check(
    "added Visayas load that binds Leyte is below the DICT 1.5 GW forecast",
    bt["available"] and 0 < bt["added_visayas_load_to_bind_leyte_mw"] < 1500,
)

# The must-run coal tranche lowers the modeled overnight price. Its inputs are
# sourced: 40% minimum stable load and the H1 2025 WESM average offer. It must
# not worsen mean absolute error or change the evening scarcity difference.
uc = disp["unit_commitment"]
check(
    "unit-commitment layer sourced (min-load + offer both carry a src)",
    uc.get("src_min_load")
    and uc.get("src_offer")
    and math.isclose(uc["min_load_frac"], 0.40)
    and math.isclose(uc["commit_offer_php_kwh"], 4.14),
)
ucg = uc["per_grid"]
check(
    "commitment never worsens MAE on any grid",
    all(
...
check(
    "commitment lifts correlation where the grid's demand dips below the "
...
check(
    "commitment lifts Luzon correlation too",
    ucg["luzon"]["correlation_after"] > ucg["luzon"]["correlation_before"],
)
check(
    "commitment lowers the modeled overnight (mean drops or holds on every grid)",
    all(
        ucg[g]["modeled_mean_after_php_kwh"]
        <= ucg[g]["modeled_mean_before_php_kwh"] + 1e-9
        for g in ucg
    ),
)
# Keep the evening-peak price gap: commitment affects only light load. The gap
# measures scarcity and offer premiums, so a past-price adjustment must not absorb it.
check(
    "commitment leaves the recorded evening price gap in place",
...
# distribution. Forced-outage rates come from NERC GADS for coal and gas.
mc = disp["reliability_mc"]
check(
    "reliability MC present + labeled simplified + seeded + sourced FOR",
    "simplified" in mc.get("method", "")
    and mc.get("draws", 0) >= 2000
    and mc.get("seed") is not None
    and mc.get("src_for")
...
mcg = mc["per_grid"]
check(
    "MC reports a distribution per grid (LOLP + expected + tail shortfall)",
    all(
        all(
            k in mcg[g]
            for k in (
...
check(
    "MC LOLP is a probability in [0, 100] on every grid",
    all(0 <= mcg[g]["lolp_pct"] <= 100 for g in mcg),
)
check(
    "baseline evening-peak grid is adequate (Luzon LOLP under 1% in the window)",
    mcg["luzon"]["lolp_pct"] < 1,
)
mcd = mc["dict_2028_luzon"]
check(
    "added-demand scenario is a labeled 1.5 GW DICT forecast with a source",
    mcd["added_mw"] == 1500 and mcd["owner"] == "DICT" and mcd.get("src"),
)
check(
    "the DICT demand forecast raises loss-of-load probability above baseline",
    mcd["distribution"]["lolp_pct"] > 2 * mcg["luzon"]["lolp_pct"]
    and mcd["distribution"]["lolp_pct"] > mcg["luzon"]["lolp_pct"],
)
check(
    "the DICT demand forecast has a nonzero 99th-percentile or maximum shortfall",
    mcd["distribution"]["shortfall_mw_p99"] > 0
    or mcd["distribution"]["shortfall_mw_max"] > 500,
)

# Storage moves energy from low-price to high-price hours.
st = disp["storage"]
check(
    "storage assets sourced (batteries + Kalayaan pumped hydro, both with src)",
    st.get("src_bess")
    and st.get("src_pumped_hydro")
    and st["assets"]["luzon"]["bess_mw"] == 634
    and st["assets"]["luzon"]["pumped_hydro_mw"] == 685
    and st["assets"]["luzon"]["total_mw"] == 1319,
)
check(
    "storage discharge offer sits above the charge floor (round-trip loss)",
    st["discharge_offer_php_kwh"] > 4.14 and st["round_trip_eff"] < 1,
)
pp = st["dict_wave_peak_price"]
check(
    "storage lowers the tight-evening price with DICT demand from oil to coal",
...
check(
    "storage lowers loss-of-load probability with the DICT demand forecast",
    rbk["luzon_dict_2028"]["with_storage"]["lolp_pct"]
    < rbk["luzon_dict_2028"]["without"]["lolp_pct"],
)
check(
    "storage also lowers expected unserved energy with the DICT demand forecast",
    rbk["luzon_dict_2028"]["with_storage"]["eue_mwh_evening_window"]
    < rbk["luzon_dict_2028"]["without"]["eue_mwh_evening_window"],
)

# Price-duration curve and marginal-block frequency.
pdur = disp["price_duration"]
check(
    "price-duration curves generated for 3 grids (modeled + observed)",
    set(pdur) == {"luzon", "visayas", "mindanao"}
    and all(pdur[g]["modeled"] and pdur[g]["observed"] for g in pdur),
)
lz_dur = pdur["luzon"]
check(
    "duration curves are sorted high to low (monotone non-increasing)",
    all(
...
check(
    "observed price-duration has a scarcity tail the cost stack never reaches",
    lz_dur["observed"][0]["price"] > lz_dur["modeled"][0]["price"],
)
check(
    "observed price-duration spans wider than the modeled plateau",
...
check(
    "marginal-frequency table generated for 3 grids, shares near 100%",
    all(
        abs(sum(b["share_pct"] for b in mfreq[g]["by_block"]) - 100) < 1.5
        for g in mfreq
    ),
)
check(
    "coal is the dominant marginal block on Luzon",
    mfreq["luzon"]["by_block"][0]["block"].startswith("coal"),
)
check(
    "the committed-coal tranche is marginal on Visayas (the commitment layer at "
    "light load)",
    any(b["block"] == "coal (committed)" for b in mfreq["visayas"]["by_block"]),
)

# --- scenario engine inputs (interactivity, ported to studio/src/studio/engine.ts) -
mo = disp["merit_order"]
check(
    "merit_order carries pre-split per-fuel availability for the client engine",
    all("fuel_avail_mw" in mo[g] and mo[g]["fuel_avail_mw"] for g in mo),
)
check(
    "solar availability at the evening reference hour is ~0 (the add-solar caveat)",
...
check(
    "scenario_golden stores at least 4 comparison cases from the coupled engine",
    len(gold["cases"]) >= 4,
)
_base = next(c for c in gold["cases"] if "baseline" in c["label"])
check(
    "golden baseline clears all three grids on the coal margin (~P6)",
    all(
        abs(_base["expect"]["price"][g] - 6.0) < 0.5
        for g in ("luzon", "visayas", "mindanao")
    ),
)
_bind = next(c for c in gold["cases"] if "binds" in c["label"])
check(
    "golden added-Visayas-load case saturates the Leyte-Luzon link at a rent",
    _bind["expect"]["leyte_saturated"] and _bind["expect"]["leyte_rent_php_kwh"] > 0,
)
hy = disp["assumptions"]["hydrology"]
check(
    "dry hydrology multiplier cuts hydro (below 1) and wet lifts it (above 1)",
    hy["dry_multiplier"] < 1 < hy["wet_multiplier"],
)
check(
    "dry hydrology reproduces the sourced DOE 2024 El Nino hydro availability",
    abs(
        hy["dry_multiplier"] * hy["modeled_normal_hydro_avail_mw"]
        - hy["dry_avail_mw_national"]
    )
    < 2,
)
...
check(
    "reserve.json is available with the four reserve products",
    res["available"]
    and {c["category"] for c in res["categories"]}
    == {"regulation_up", "regulation_down", "contingency", "dispatchable"},
)
check(
    "reserve prices are in a sane band (0 to the reserve cap)",
    all(
        0 <= c["mean_php_kwh"] <= res["reserve_cap_php_kwh"]
        and c["max_php_kwh"] <= res["reserve_cap_php_kwh"] + 0.1
        for c in res["categories"]
    ),
)
check(
    "regulation is the dearest reserve product (scarcest ancillary service)",
    res["categories"][0]["category"].startswith("regulation"),
)
check(
    "the dearest reserve product clears above the observed energy price "
...
check(
    "reserve prices generated per grid for all three grids",
    set(res["by_grid"]) == {"luzon", "visayas", "mindanao"},
)

# Contract coverage and household-bill effect.
bill = load("bill.json")
check(
    "bill.json is available with the Meralco supply mix summing to 100%",
    bill["available"] and abs(sum(bill["supply_mix_pct"].values()) - 100) < 0.5,
)
check(
    "WESM is the minority (residual) slice of the supply mix",
    bill["supply_mix_pct"]["wesm"] == min(bill["supply_mix_pct"].values()),
)
check(
    "the WESM pass-through factor equals the WESM supply share (not full spot)",
    abs(bill["pass_through_factor"] - bill["wesm_share_pct"] / 100) < 1e-9
    and bill["pass_through_factor"] < 0.5,
)
check(
    "the WESM contribution to the generation charge is share x price",
    math.isclose(
    ...
    < bill["generation_charge_php_kwh"]
...
_contracted_php_kwh = (
    bill["generation_charge_php_kwh"] - bill["wesm_contribution_to_gen_charge_php_kwh"]
) / (1 - bill["pass_through_factor"])
check(
    "contracted generation implies a physically possible unit price",
...
check(
    "market_power.json is available with the ERC capacity shares",
    mp["available"] and len(mp["companies"]) >= 5,
)
check(
    "named shares plus others sum to 100%",
    abs(sum(c["share_pct"] for c in mp["companies"]) + mp["others_share_pct"] - 100)
    < 0.5,
)
check(
    "HHI floor <= ceiling and the floor is the sum of squared named shares",
    mp["hhi_floor"] <= mp["hhi_ceiling"]
    and abs(mp["hhi_floor"] - sum(c["share_pct"] ** 2 for c in mp["companies"])) < 0.5,
)
check(
    "the largest firm stays under the EPIRA market-share cap",
    mp["largest"]["share_pct"] < mp["cap_demand_pct"],
)
check(
    "the two largest firms are close to half of national capacity",
    40 < mp["top2_combined_pct"] < 50,
)

# --- observed day profiles + chronological engine (studio maturation, phase 1) ---
prof = load("profiles.json")
check("profiles carries at least 60 replayable observed days", len(prof["days"]) >= 60)
check(
    "every profile day has full 24-hour demand on all three grids",
    all(
        all(
            len(d["demand"][g]) == 24 and all(v is not None for v in d["demand"][g])
            for g in ("luzon", "visayas", "mindanao")
        )
        for d in prof["days"]
    ),
...
check(
    "default and stress days exist and are market days",
    prof["default_day"] in dates
    and prof["stress_day"] in dates
    and all(
        next(d for d in prof["days"] if d["date"] == x)["market"]
        for x in (prof["default_day"], prof["stress_day"])
    ),
)
check(
    "solar profile is a 24-hour shape peaking midday under 1.0",
    len(prof["solar_profile"]) == 24
    and 0 < max(prof["solar_profile"]) < 1
    and prof["solar_profile"][12] > prof["solar_profile"][19],
)
check(
    "storage defaults pin the sourced powers (BESS 634, Kalayaan 685)",
    {s["id"]: s["power_mw"] for s in prof["storage_defaults"]}
    == {"bess_luzon": 634, "kalayaan": 685},
)
check(
    "storage energies are labeled assumptions",
    all("ASSUMPTION" in s["energy_note"] for s in prof["storage_defaults"]),
)
check(
    "reserve requirement means are positive per grid",
    all(v > 0 for g in prof["reserve_req_mean_mw"].values() for v in g.values()),
)

cg = prof["chrono_golden"]
check(
    "chrono golden has 11 cases (8 cost-mode + 3 offer-mode) of 24 hourly "
    "prices per grid",
    cg["available"]
    and len(cg["cases"]) == 11
    and all(
...
check(
    "the offer-mode goldens carry the marker both engines resolve",
...
check(
    "flat DC load never lowers the mean Luzon price (monotonicity)",
    dc_mean >= base_mean,
)
check(
    "the default storage fleet charges and discharges in its reference case",
    max(cg["cases"][5]["expect"]["soc_mwh"]) > 0
    and abs(cg["cases"][5]["expect"]["soc_mwh"][23]) < 1e-9,
)
check(
    "golden tolerances are the parity contract",
    math.isclose(cg["tolerance_php_kwh"], 0.02)
    and math.isclose(cg["tolerance_mw"], 1.0),
)

# The 168-hour case carries seven explicit dates in the input
...
wk = cg.get("week", {})
check(
    "week golden generated with seven explicit dates and a 168-price expect",
    wk.get("available")
    and len(wk["input"]["dates"]) == 7
    and all(
        len(wk["expect"]["price"][g]) == 168 for g in ("luzon", "visayas", "mindanao")
    ),
)
check(
    "week storage carries across midnight (a day ends with charge to spare)",
    max(d["end_soc_mwh"] for d in wk["expect"]["days"]) > 1,
)

bc = prof["backcast"]
check("backcast replays at least 30 market days", bc["available"] and bc["days"] >= 30)
check(
    "backcast hours = days x 24 per grid",
    all(v["n_hours"] == bc["days"] * 24 for v in bc["per_grid"].values()),
)
check(
    "backcast reports its miss: model under-prices the observed mean (negative bias)",
    all(v["bias_php_kwh"] < 0 for v in bc["per_grid"].values()),
)
check(
    "backcast never fakes a hit rate from a flat model",
    all(
...
check(
    "fleet parses all three grids and reconciles to the DOE subtotals",
    fleet["available"]
    and all(
        v["ok"]
        for g in fleet["editions"].values()
        for v in g["reconciliation"].values()
    ),
)
check(
    "fleet editions are the 2025 DOE lists with Wayback capture URLs",
    all(
        e["as_of"] >= "2025-03-31"
        and "web.archive.org" in e["src"]
        and "doe.gov.ph" in e["original_url"]
        for e in fleet["editions"].values()
    ),
)
check(
    "fleet section sums equal the DOE grand total per grid",
    all(
...
plants = fleet["plants"]
check(
    "both Sual units are in the fleet at 647 MW each",
    [p["installed_mw"] for p in plants if p["name"] in ("SPI U1", "SPI U2")]
    == [647.0, 647.0],
)
check(
    "every plant has a known fuel and positive installed MW",
    all(
        p["fuel"]
        in (
            "coal",
        ...
        and p["installed_mw"] > 0
        for p in plants
    ),
)
check(
    "plant ids are unique per grid",
    len({(p["grid"], p["name"]) for p in plants}) == len(plants),
)

# The DOE Power Development Plan demand path used by the long-term view.
dpath = load("demand_path.json")
check(
    "DOE PDP demand path generated: per-grid peak forecast, labeled DOE owner",
    dpath.get("available")
    and dpath["owner"] == "DOE"
    and "2023-2050" in dpath["plan"]
    and len(dpath["years"]) >= 25,
)
check(
    "every PDP year reconciles: the three grids sum to the plan's own "
    "Philippines total within 2 MW (the parse gate held)",
    all(
        abs(
            dpath["per_grid_mw"]["luzon"][i]
            + dpath["per_grid_mw"]["visayas"][i]
            + dpath["per_grid_mw"]["mindanao"][i]
            - dpath["philippines_mw"][i]
        )
        <= 2
...
check(
    "the demand path is a monotone growing forecast at the DOE's stated "
...
# Long-term plan: DOE project lists and transmission-plan corridors.
pj = load("projects.json")
check(
    "projects available with the 2025-12-31 edition",
    pj["available"] and pj["as_of"] == "2025-12-31",
)
check(
    "projects reconcile to the DOE LVM summary totals",
    math.isclose(pj["totals"]["committed"]["lvm_gen_mw"], 13839.48, abs_tol=0.05)
    and math.isclose(pj["totals"]["indicative"]["lvm_gen_mw"], 119226.39, abs_tol=0.05),
)
check(
    "committed per-grid totals pin to the DOE summary",
    math.isclose(pj["totals"]["committed"]["luzon"]["gen_mw"], 11875.89, abs_tol=0.02)
    and math.isclose(
        pj["totals"]["committed"]["visayas"]["gen_mw"], 1443.75, abs_tol=0.02
    )
    and math.isclose(
        pj["totals"]["committed"]["mindanao"]["gen_mw"], 519.85, abs_tol=0.02
    ),
)
check(
    "every project row carries grid, status, fuel, positive MW",
    all(
...
check(
    "reconciled sections' rows sum to their printed subtotals",
    all(
        s["rows_reconciled"] is False
        or math.isclose(
            sum(
                r["mw"]
                for r in pj["rows"]
...
check(
    "every edition carries a Wayback capture of the DOE URL",
    all(
        "web.archive.org" in e["src"] and "doe.gov.ph" in e["original_url"]
        for st in pj["editions"].values()
        for e in st.values()
    ),
)
check(
    "TDP corridors: MW only where the TDP states transfer capacity",
    all(
...
check(
    "TDP corridor MW pins: LV HVDC +440, MVIP +450",
...
pasa = load("pasa.json")
check(
    "pasa covers the whole OUTRTD archive",
    pasa["available"] and len(pasa["days"]) == meta["datasets"]["OUTRTD"],
)
check(
    "pasa resources partition into verified/unmatched/storage",
    pasa["n_verified"] + pasa["n_unmatched"] + pasa["n_storage"] == pasa["n_resources"],
)
check(
    "every verified pasa resource carries plant, grid, fuel, MW",
    all(
        r["plant"] and r["grid"] and r["fuel"] and (r["unit_mw"] or 0) > 0
        for r in pasa["resources"]
        if r["match"] == "verified"
    ),
)
check(
    "unmatched pasa resources carry no MW (a floor, never a guess)",
    all(r["unit_mw"] is None for r in pasa["resources"] if r["match"] != "verified"),
)
pasa_by_name = {r["resource"]: r for r in pasa["resources"]}
check(
    "pasa day MW equals its verified resources",
    all(
        math.isclose(
            sum(day["matched_mw"].values()),
            sum(
                pasa_by_name[name]["unit_mw"] or 0
                for name in day["out"]
...
check(
    "pasa states its coverage and the inferred grid mapping",
...
check(
    "emissions factors all carry a basis and a source",
    all(f["basis"] and f["src"] for f in em["factors"]),
)
check(
    "emissions fossil factors pinned to the sourced derivations",
...
check(
    "emissions: renewables and storage zero operational, biomass excluded",
    em["factor_map"]["hydro"] == 0
    and em["factor_map"]["solar"] == 0
    and em["factor_map"]["storage"] == 0
    and "biomass" not in em["factor_map"],
)
check(
    "emissions NGEF anchor present with source",
...
check(
    "bill mix history carries Apr-Jun 2026 in order",
    [m["period"] for m in hist] == ["2026-04", "2026-05", "2026-06"],
)
check(
    "bill mix shares sum to 100 per month",
    all(m["wesm_pct"] + m["psa_pct"] + m["ipp_pct"] == 100 for m in hist),
)
check(
    "bill June row matches the standing anchors",
    bool(hist)
    and hist[-1]["wesm_pct"] == 10
    and math.isclose(hist[-1]["generation_charge_php_kwh"], 9.0704)
    and math.isclose(hist[-1]["total_rate_php_kwh"], 14.4833),
)
check(
    "bill history rows each carry advisory + news sources",
    all(m["src"] and m["src_news"] for m in hist),
)
...
ps = mo["price_setters"]
check(
    "price setters generated from a full MCP window",
    ps["available"] and ps["days"] >= 80,
)
check(
    "every grid names observed setters with shares that are shares",
    all(
...
check(
    "setter fuel matching is majority, never claimed complete",
    all(
...
check(
    "official reserve prices span the window with stats",
    rp["available"] and len(rp["dates"]) >= 80 and rp["stats"]["luzon"],
)
adv = mo["advisories"]
check(
    "advisory stream generated with HVDC events captured",
    adv["available"] and len(adv["days"]) >= 80 and len(adv["hvdc_events"]) > 0,
)
ol = mo["outlook"]
check(
    "week-ahead outage outlook carries matched MW and explicit gaps",
    ol["available"]
    and ol["as_of"] >= "2026-07-01"
    and all(g in ol["matched_mw"] for g in ("luzon", "visayas", "mindanao")),
)
# The unit-commitment case uses block-level minimum stable output. That level of
# detail worsens price correlation, so the linear program remains the default.
uc = mo["uc_probe"]
check(
    "UC probe generated with the generic labeled min-stable and a verdict",
    uc.get("engine_default") == "lp"
    and uc["min_stable_generic"]["coal"] > 0
    and "generic" in uc["min_stable_label"],
)
check(
    "commitment worsens the Luzon LWAP price correlation (LP stays default)",
    uc["corr_delta"]["lwap"]["luzon"]["delta"] < 0,
)

drv = load("drivers.json")
check(
    "drivers timeline joins one row per archive day",
    drv["available"] and len(drv["days"]) >= 80,
)
check(
    "drivers rows carry the observed columns",
    all(
...
check(
    "every market_ops section carries the analytics disclaimer",
    all(
        "disclaimer" in s
        for s in (
            ps,
...
no = mo["not_offered"]
check(
    "the not-offered screen is generated, bounded, and self-disclaiming",
    no["available"]
    and len(no["days"]) >= 30
    and all(
        0 <= r[g]["not_offered_mw"] <= r[g]["registered_mw"]
        for r in no["days"]
        for g in r
        if g != "date"
    )
...
check(
    "the operator's own dispatch cut is generated for all three grids (MOT: "
    "288 five-minute intervals per grid per day, 15+ days spread across "
    "the window so the headroom read is not a 3-day sampling effect)",
    mot.get("available")
    and mot["days"] >= 15
    and all(mot["per_grid"][g]["n_intervals"] == 288 * mot["days"] for g in _mot_grids),
)
check(
    "published headroom is a real bounded quantity on every grid: MW "
    ...
    all(
        0
        <= mot["per_grid"][g]["headroom_mw"]["min"]
...
check(
    "the cut parse reproduces the operator's named price setter well "
    "clear of a random draw (the rate is meaningless without the null)",
    all(
        mot["per_grid"][g]["mcp_agreement_pct"] >= 85
...
check(
    "MOT dispatched MW is cleared, not as-bid: it reconciles to the "
    "RTDSUM generation the operator reports for the same interval",
    all(abs(mot["per_grid"][g]["rtdsum_ratio"] - 1.0) < 0.05 for g in _mot_grids)
...
check(
    "the Luzon MOT gap is not import (import is a small fraction of it) "
    "and equals RTDSUM's own energy-balance residual",
    _lzp.get("gap_mw_mean")
    and _lzp.get("import_mw_mean")
...
check(
    "market days carry the outage deviation layer",
    all(d.get("out_dev_mw") is not None for d in mkt_days),
)
check(
    "hydro and storage never appear in outage deviations",
    all(
        f not in ("hydro", "storage")
        for d in mkt_days
        for g in (d.get("out_dev_mw") or {})
        for f in (d["out_dev_mw"][g] or {})
    ),
)
check(
    "most market days carry the observed MCP series",
    sum(1 for d in mkt_days if d.get("mcp")) >= 0.8 * len(mkt_days),
)
bc = prof["backcast"]
check(
    "backcast scores both targets (LWAP and MCP)",
    bc["per_grid"]
    and bc.get("per_grid_mcp")
    and set(bc["per_grid_mcp"]) == {"luzon", "visayas", "mindanao"},
)
check(
    "backcast scores the corridors (flows table)",
    bc.get("flows")
    and set(bc["flows"]) == {"lv", "vm"}
    and all(
...
check(
    "offer-mode backcast generated from the derived offer days",
    ob.get("available")
    and ob["days"] >= 30
...
check(
    "offer mode carries the no-extra-layers statement",
    "no storage" in (ob.get("note") or ""),
)
check(
    "offer mode improves corridor-direction agreement over the cost calculation",
    (ob.get("flows") or {}).get("vm", {}).get("direction_agreement_pct", 0)
    > (bc["flows"]["vm"]["direction_agreement_pct"] or 0),
)
# The model difference the README quotes: on the reference day, added demand must
...
cost_delta = _mp("DICT 1.5 GW flat load on Luzon", "luzon") - _mp(
    "base day, no storage", "luzon"
)
offer_delta = _mp("DICT 1.5 GW on the observed offer book", "luzon") - _mp(
    "observed offer book, unchanged scenario", "luzon"
)
check(
    "the added-demand price change is larger on the observed bids than the cost "
    "stack (cost-mode scenario deltas are floors)",
    offer_delta > cost_delta > 0,
)
check(
    "the as-bid shock reaches the other grids",
    all(
        (
            _mp("DICT 1.5 GW on the observed offer book", g)
            - _mp("observed offer book, unchanged scenario", g)
        )
        > 0.5
...
check(
    "days with observed HVDC blocks carry corridor availability fractions",
    len(capped_days) >= 20
    and all(
...
# These archive-size floors only grow as the daily job adds files.
RAW_DIR = os.path.join(os.path.dirname(__file__), "..", "data", "raw")
with open(os.path.join(RAW_DIR, "manifest.json")) as _fh:
    manifest = json.load(_fh)
check(
    "GWAPF (secondary-cap trigger series) is archived across its window",
    (manifest.get("GWAPF") or {}).get("files", 0) >= 70,
)
check(
    "RTDHS (observed HVDC corridor record) is archived across its window",
    (manifest.get("RTDHS") or {}).get("files", 0) >= 85,
)
RES_DIR = os.path.join(
    os.path.dirname(__file__), "..", "data", "derived", "reserve_daily"
)
res_days = sorted(n for n in os.listdir(RES_DIR) if n.startswith("RESD_"))
check("reserve books (RTDOR) derived across the offer window", len(res_days) >= 55)
with open(os.path.join(RES_DIR, res_days[-1])) as _fh:
    resd = json.load(_fh)
check(
    "newest reserve book: four commodities x three grids x 24 hours",
    all(
        len(resd["hours"][g][c]) == 24
        for g in ("luzon", "visayas", "mindanao")
        for c in ("Fr", "Dr", "Ru", "Rd")
    ),
)
check(
    "newest reserve book prices sit inside the published reserve cap",
    all(
        0.0 <= p <= res["reserve_cap_php_kwh"] + 0.001
        for g in resd["hours"].values()
        for c in g.values()
        for h in c
        for p, _ in (h or [])
    ),
...
check(
    "newest reserve book honors the gate (book covers opening schedule)",
    all(
        sum(m for _, m in (resd["hours"][g][c][h] or []))
...
# The three archived datasets must feed the derived market measures.
mo = load("market_ops.json")
rv = mo.get("reserve_validation") or {}
check(
    "reserve replay scored across the derived window (80+ days)",
...
check(
    "reserve replay covers all 12 grid-commodity pools",
    len(_rv_pools) == 12 and all(p["n_hours"] > 1500 for p in _rv_pools),
)
check(
    "the co-optimisation wedge is one-signed: every pool's bias is "
    "negative (the book-only replay under-prices reserves everywhere)",
    all(p["bias_php_kwh"] < 0 for p in _rv_pools),
)
check(
    "the wedge dominates to aggregate precision (MAE equals -bias "
    "within half a centavo in every pool)",
    all(abs(p["mae_php_kwh"] + p["bias_php_kwh"]) <= 0.005 for p in _rv_pools),
)
check(
    "hours where the marginal offer sits above the official price "
    "stay noise-level (the prose's quantified concession)",
    rv["hours_model_above_pct"] <= 10.0 and rv["max_model_above_php_kwh"] <= 0.05,
)
check(
    "Luzon dispatchable reserve reproduces the official price "
...
check(
    "reserve replay states its scarcity accounting and observes "
    "scarcity somewhere in the window",
    "scarcity" in (rv.get("scarcity_note") or "")
    and any(p["n_scarcity_hours"] > 0 for p in _rv_pools),
)

# Settlement-side prices and final reserve results.
# DIPCRF provides per-resource cleared reserve for the tighter reserve check.
rr = mo.get("reserve_results") or {}
check(
    "per-resource reserve results are consumed (DIPCRF: 70+ days, the "
    "resource identity the pooled replay and RSVPR both drop)",
    rr.get("available") and rr["days"] >= 70 and rr["resources_named"] >= 150,
)
_rr_pools = [
    rr["pools"][g][c]
    for g in ("luzon", "visayas", "mindanao")
...
check(
    "the book replay under-prices the FINAL cleared price on every pool "
    "too: the co-optimisation wedge holds against the authoritative target",
    len(_rr_pools) == 12
    and all(
        p["replay_vs_final"]["bias_php_kwh"] < 0
        for p in _rr_pools
        if "replay_vs_final" in p
    ),
)
_rr_grids = ("luzon", "visayas", "mindanao")
check(
    "the final re-solve's reserve-schedule revision stays a few MW across "
    ...
    all(
        abs(rr["pools"][g][c]["final_vs_rtd_schedule_mw"]["mean_daily_revision"]) < 8.0
        for g in _rr_grids
        for c in ("Fr", "Dr", "Ru", "Rd")
    )
    and abs(
        rr["pools"]["luzon"]["Fr"]["final_vs_rtd_schedule_mw"]["mean_daily_revision"]
    )
    < 1.0
    and abs(
        rr["pools"]["visayas"]["Dr"]["final_vs_rtd_schedule_mw"]["mean_daily_revision"]
    )
    > 2.0,
)
check(
    "the final-vs-RTD reserve-price gap concentrates on the regulation "
    "products (Ru/Rd), not contingency (the corrected framing)",
    max(
        abs(rr["pools"][g][c]["vs_rtd_price"]["bias_php_kwh"])
        for g in _rr_grids
        for c in ("Ru", "Rd")
    )
    > max(abs(rr["pools"][g]["Fr"]["vs_rtd_price"]["bias_php_kwh"]) for g in _rr_grids),
)
# CAPER provides registered ancillary-services capacity for comparison.
rreg = mo.get("reserve_registration") or {}
check(
    "registered ancillary-services capacity gives the reserve book a "
    "registration base (CAPER: 3 grids x 4 commodities, 80+ days)",
    rreg.get("available")
    and len(rreg["days"]) >= 80
    and all(
        c in (rreg.get("stats", {}).get(g) or {})
        for g in ("luzon", "visayas", "mindanao")
        for c in ("Fr", "Dr", "Ru", "Rd")
    ),
)
check(
    "the tight Visayas dispatchable reserve offers most of its "
...
check(
    "the settlement-side sample is consumed (administered, settlement, "
    "and day-ahead on market days)",
    ss.get("available")
    and len(ss.get("sample_days") or []) >= 3
    and all(g in _ssg for g in ("luzon", "visayas", "mindanao")),
)
check(
    "the settlement congestion component is empty at regional "
    "granularity (no receipt beyond the flows table)",
    all(
        _ssg[g]["settlement_congestion_max_abs_php_kwh"] <= 0.01
        for g in ("luzon", "visayas", "mindanao")
    ),
)
check(
    "the administered price carries the island premium (Visayas and "
    "Mindanao administered prices run above Luzon's, a cost-based "
    "counterfactual not the flat cost floor)",
    _ssg["visayas"]["admin_lmp_mean_php_kwh"] > _ssg["luzon"]["admin_lmp_mean_php_kwh"]
    and _ssg["mindanao"]["admin_lmp_mean_php_kwh"]
    > _ssg["luzon"]["admin_lmp_mean_php_kwh"],
)
check(
    "the day-ahead projection spread is both-signed (a projection "
    "diagnostic, out of the replay's dispatch scope)",
    _ssg["luzon"]["dap_vs_rt_spread_range_php_kwh"][0]
    < 0
    < _ssg["luzon"]["dap_vs_rt_spread_range_php_kwh"][1],
)
# Recorded solar and wind dispatch by grid, and the
# measured result that the observed/model ratio flips by grid (the model's
# solar SPLIT is approximate, not a single curtailment gap)
sw = mo["solar_wind_observed"]
check(
    "observed dispatched solar/wind consumed per grid (DIPCEF); the "
...
check(
    "solar dominates observed dispatched renewables (Luzon solar many times its wind)",
...
check(
    "the two observed corridor records agree (identity vs RTDHS "
...
check(
    "the operator's binding shares are real fractions of the record",
    all(
        v["binding_share_pct"] is not None
        and 20.0 <= v["binding_share_pct"] <= 80.0
        and v["n_intervals"] > 20000
        for v in fr["corridors"].values()
    ),
)
...
for _mode in ("backcast", "offer_backcast"):
    ...
    check(
        f"{_mode} scores its corridor flows against the operator's "
...
check(
    "offer mode keeps the corridor story against the independent "
    "record (vm direction 95%+, MAE under 80 MW)",
    _ofx["vm"]["direction_agreement_pct"] >= 95.0 and _ofx["vm"]["mae_mw"] <= 80.0,
)
check(
    "the under-binding gap is published: the operator flagged the "
    "corridors binding more often than the replay hits the cap",
    all(
        _ofx[k]["observed_binding_share_pct"] > _ofx[k]["modeled_at_cap_share_pct"]
        for k in ("lv", "vm")
    ),
)
...
check(
    "the ramp probe measured the published curves, not asserted them",
    rp.get("available")
    and rp["n_resources_with_published_ramp"] > 100
    and rp["hourly_inert_pct"] is not None,
)
check(
    "the fleet out-ramps the worst demand rise on the STRICT basis on every "
    "grid (online units, slowest band, worst sampled hour: the read the "
    "verdict rests on, not the flattering offered-best one)",
    rp.get("verdict") == "does_not_bind_at_hourly_resolution"
    and all(
...
check(
    "the ramp probe states its own scope rather than overclaiming",
    "HOURLY" in (rp.get("note") or "") and "per-unit" in (rp.get("note") or "").lower(),
)

gt = mo.get("gwap_trigger") or {}
check(
    "the secondary-cap trigger series covers all five published GWAPF "
    "rows, including the combined Luzon-Visayas region",
    gt.get("available")
    and abs(gt["trigger_php_kwh"] - 12.413) < 1e-9
    and abs(gt["cap_php_kwh"] - 7.423) < 1e-9
    and set(gt["per_region"])
    == {"luzon", "visayas", "mindanao", "system", "luzon_visayas"}
    and all(v["max_rolling_72h"] is not None for v in gt["per_region"].values()),
)
...
check(
    "above-offer-cap intervals are counted, not silently averaged in",
    all("n_intervals_above_offer_cap" in v for v in gt["per_region"].values())
    and _lz["n_intervals_above_offer_cap"] > 0,
)
check(
    "held at the offer cap, Luzon breaches the trigger zero times and "
    ...
    < gt["trigger_php_kwh"]
    < _lz["max_rolling_72h"]["rolling_php_kwh"],
)
check(
    "the clamp scan found no day pinned at either cap level, which is "
    "consistent with the offer-cap-held series never breaching in Luzon",
    gt.get("clamp_scan_days_pinned") == {"7.423": 0, "6.245": 0},
)
check(
    "the reference as-bid scenario raises the rolling series above its own "
    "baseline (the scenario's effect, whether or not it trips)",
    (gt.get("reference_case") or {}).get("scenario_max_rolling_72h")
    and gt["reference_case"]["scenario_max_rolling_72h"]["rolling_php_kwh"]
    > gt["reference_case"]["baseline_max_rolling_72h"]["rolling_php_kwh"]
    and gt["reference_case"]["baseline_trips"] is False,
)
# The named day must remain under the threshold stated in three public outputs.
# This check fails if regenerated data move it above the threshold.
check(
    "the reference day still lands UNDER the trigger, as the prose says",
    gt["reference_case"]["trips_trigger"] is False
    and gt["reference_case"]["scenario_max_rolling_72h"]["rolling_php_kwh"]
    < gt["trigger_php_kwh"],
)
check(
    "the trigger block still discloses the mechanics it cannot reproduce",
    "does not\nreproduce" in (gt.get("mechanism_note") or "").replace(" ", " ")
    or "does not" in (gt.get("mechanism_note") or ""),
)
check(
    "the trigger block states which row is operative and names the "
...
# Constrained-on units and security limits must appear in the market measures.
co = mo.get("constrained_on") or {}
check(
    "constrained-on roster generated across its published window",
    co.get("available") and co["n_days"] >= 70 and co["n_resources"] >= 100,
)
check(
    "constrained-on names units in all three grids with prices inside the offer cap",
    all(co["per_grid_intervals"][g] > 0 for g in ("luzon", "visayas", "mindanao"))
    and all(
        0 < r["mean_price_php_kwh"] <= 32.0
...
check(
    "constrained-on states what it is (administered, not market "
    "clearing; two-week lag)",
    "administered" in (co.get("note") or "") and "two weeks" in (co.get("note") or ""),
)
sl = mo.get("security_limits") or {}
check(
    "security limits generated across their published window",
    sl.get("available")
    and sl["n_days"] >= 85
    and sl["n_windows"] > 10000
    and sl["n_resources"] >= 10,
)
check(
    "the security limits are pinned operating points (MAX equals MIN "
...
check(
    "security-limit resources resolve to a grid from RTDSL region codes",
    all(r["grid"] in ("luzon", "visayas", "mindanao") for r in sl["top"]),
)

# The system operator's dispatch instructions must appear in the market measures.
so = mo.get("so_instructions") or {}
mot = so.get("motrd") or {}
check(
    "MOT-raise record generated across its weekly window",
    so.get("available")
    and len(mot["weeks"]) >= 10
...
check(
    ...
    all(r["grid"] in ("luzon", "visayas", "mindanao") for r in mot["top"]),
)
sod = so.get("sodir") or {}
check(
    "the daily instruction log spans the window with the operator's "
    "own categories (weekly compilations archived, never counted)",
    sod["n_files"] >= 260
    and sod["n_days"] >= 85
    and sod["n_weekly_files_archived"] >= 30
    and sod["n_instructions"] > 50000
    and len(sod["categories"]) >= 5,
)
check(
    "Leyte-Cebu accounts for at least 95% of recorded line-limitation remarks",
    sod["n_limitation_remarks"] > 1000
    and sod["limitation_causes"]["leyte-cebu"] > 1000
    and 100 * sod["limitation_causes"]["leyte-cebu"] / sod["n_limitation_remarks"]
    >= 95.0,
)
check(
    "the discrepancy list travels with the family (newest revision per week)",
    (so.get("discrepancies") or {}).get("n_weeks", 0) >= 5
    and so["discrepancies"]["n_rows_newest_revisions"] > 0,
)
# The administered-dispatch check shows that MOT-raise instructions move MW but
# do not raise prices in this fuel-level model. Coal already sets the price in
# about 90 percent of affected hours.
ad = mo.get("admin_dispatch") or {}
check(
    "the administered-dispatch check moves MW but does not raise prices "
    "in the fuel-level engines (coal is marginal on about 90 percent of "
    "raise hours; every raise hour already prices at coal's floor so a "
    "floor cannot lift it), documented not built",
    ad.get("available")
    and ad["verdict"] == "does_not_raise_prices_in_fuel_level_model"
...
check(
    "the administered raises are material (several percent of dispatch on "
    "every grid's raise hours, not the inert must-run subset)",
    all(
        ad["mw_weighted_fraction_of_dispatch"][g]["mw_weighted_pct"] >= 3.0
        for g in ("luzon", "visayas", "mindanao")
    ),
)
# The tested joint energy-and-reserve calculation does not reproduce the official
# reserve price. Public offers do not explain the scarcity charge.
jl = mo.get("joint_lp_probe") or {}
check(
    "the tested joint energy-and-reserve calculation shows that the difference is "
...
check(
    "on the requirement-met hours the offer stack cleared to the scheduled "
...
# Negative prices occur when supply and demand are nearly equal. Five-minute
# resolution is necessary, but the offers still do not resolve the margin.
sh = mo.get("subhourly_probe") or {}
check(
    "negative prices occur with floor-priced supply within a few percent "
    "of native load, while the observed regional price "
    "crosses negative, but the margin is finer than the offers resolve",
    sh.get("available")
    and sh["verdict"] == "negative_prices_need_finer_supply_data_than_public_offers"
    and sh["min_margin_pct"] < 1.0
    and sh["n_days_with_observed_negatives"] >= 3,
)
check(
    "the most negative prices are local: at the lowest-price interval "
...
gj = load("grid.json")
check("grid lines generated at scale (>= 1,200 segments)", len(gl["features"]) >= 1200)
check("grid nodes generated at scale (>= 450 substations)", len(gn["features"]) >= 450)
check(
    "grid attribution names OpenStreetMap + ODbL on both layers",
    all(
        "OpenStreetMap" in (fc.get("attribution") or "")
        and "ODbL" in (fc.get("attribution") or "")
        for fc in (gl, gn)
    ),
)
kinds = {}
...
check(
    "all five line classes present (500/230/138/hvdc/cable)",
    kinds.get("ac500", 0) >= 100
    and kinds.get("ac230", 0) >= 500
    and kinds.get("ac138", 0) >= 200
    and kinds.get("hvdc", 0) >= 10
    and kinds.get("cable", 0) >= 15,
)
in_ph = all(
    4.4 <= lat <= 21.6 and 116.4 <= lon <= 127.3
    for f in gl["features"]
...
check("nodal_obs generated and available", no.get("available") is True)
check(
    "nodal_obs covers the node population (>= 1,000 nodes)",
    no.get("n_nodes", 0) >= 1000,
)
check(
    "nodal_obs places a material subset on the mapped grid (>= 200)",
    no.get("n_placed", 0) >= 200,
)
check(
    "nodal_obs per-grid stats for all three grids",
    all(
        g in no.get("per_grid", {})
        and no["per_grid"][g].get("n_nodes")
        and no["per_grid"][g].get("top")
        and no["per_grid"][g].get("bottom")
        for g in ("luzon", "visayas", "mindanao")
    ),
)
check(
    "nodal_obs placed nodes inside the PH bbox with full fields",
    all(
        4.4 <= p["lat"] <= 21.6
        and 116.4 <= p["lon"] <= 127.3
        and p.get("res")
        and p.get("grid")
        and p.get("dev") is not None
        for p in no.get("placed", [])
    ),
)
check(
    "nodal_obs clean-day criterion recorded and limits noted",
...
check(
    "resolution lift holds (>= 450 placed, >= 60% of MW resolved)",
    no.get("n_placed", 0) >= 450
    and (no.get("resolution") or {}).get("mw_share", 0) >= 0.6,
)
check(
    "every placed node carries a placement source from the ladder",
    all(
        p.get("src")
        in (
            "osm-substation",
...
check(
    "placement uses at least three distinct evidence sources",
    len((no.get("resolution") or {}).get("by_src", {})) >= 3,
)
check(
    "hourly-shape stats present (evening deviation on most nodes)",
    sum(1 for n in no.get("nodes", []) if n.get("dev_pk") is not None)
    >= 0.9 * no.get("n_nodes", 1),
)

# Loss calculation structure and stated result.
lsf = load("loss_surface.json")
check("loss surface generated and available", lsf.get("available") is True)
check(
    "loss surface covers all three grids with stated stats",
    all(
        g in lsf.get("window", {})
...
check(
    "luzon stays a validated grid (rank correlation >= 0.4)",
    "luzon" in lsf.get("validated_grids", []),
)
check(
    "the verdict is self-computing and failing grids are named, not hidden",
    bool(lsf.get("finding"))
    and set(lsf.get("validated_grids", []) + lsf.get("failing_grids", []))
    == {"luzon", "visayas", "mindanao"},
)
check(
    "loss-surface assumptions labeled (resistances are estimates)",
    "estimates" in (lsf.get("assumptions") or {}).get("note", ""),
)
check(
    "loss calculation includes per-node scatter for the figure and studio card",
    all(
        g in lsf.get("scatter", {})
        and len(lsf["scatter"][g]) >= 50
        and all(len(p) == 2 for p in lsf["scatter"][g][:5])
        for g in ("luzon", "visayas", "mindanao")
    ),
)
# The loss-comparison figure must be an animated GIF.
check(
    "the validation figure exists and is a real animation",
    os.path.isfile(
        os.path.join(os.path.dirname(__file__), "..", "docs", "loss-surface.gif")
    )
    and os.path.getsize(
        os.path.join(os.path.dirname(__file__), "..", "docs", "loss-surface.gif")
    )
...
NODAL = os.path.join(os.path.dirname(__file__), "..", "data", "derived", "nodal_daily")
...
if ndays:
    with open(os.path.join(NODAL, ndays[-1])) as f:
        nd = json.load(f)
    check(
        "nodal daily schema (regions + nodes with dev/mw arrays)",
        set(nd["regions"]) == {"luzon", "visayas", "mindanao"}
        and len(nd["nodes"]) > 1000
        and all(
            len(v["dev_php_kwh"]) == 24 and len(v["mw"]) == 24
            for v in list(nd["nodes"].values())[:20]
        ),
    )

# The typical-offer case must improve Luzon LWAP correlation and close at least
# half the gap to the same-day replay. Reserve withholding must retain its
# measured zero effect on the flat cost stack.
PMP = os.path.join(
    os.path.dirname(__file__), "..", "data", "derived", "price_model_probes.json"
)
...
if os.path.isfile(PMP):
    ...
    check(
        "stylized book improves on the cost stack (Luzon LWAP correlation)",
        pmp_styl["correlation"] - pmp_base["correlation"] > 0.1,
    )
    check(
        "stylized book closes half the gap to the same-day replay",
        (pmp["stylized_gap_closed_frac"] or 0) >= 0.5,
    )
    check(
        "stylized book lifts the within-day shape, not just the level",
        pmp_styl["within_day_corr_median"] > pmp_base["within_day_corr_median"],
    )
    check(
        "reserve withholding does not change the cost-stack result",
        abs(pmp["corr_delta_vs_base"]["reserve"]["lwap"]["luzon"] or 0) < 0.005,
    )
    check(
        "probes state the two inputs that lack separate daily series",
