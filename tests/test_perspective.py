...
numbers directly, so a drifted data build here is a wrong public claim there.
...
hand-typed, and does not re-read the data build, so a re-run that shifts a number
...
and asserts the data build's formatted values still appear in what is left.
"""
...
def check_html_static_text(c, p, s, w, wa, pr, SITE):
    ...
    has(
        "campus water range",
        f"{c['water_mld'][0]:.0f} to {c['water_mld'][1]:.0f} million liters",
    )
    ...
    has(
        "the paddy duty is named beside the hectares it produces",
    ...
    missing = round(
        c["mw"] - (w["own_station_mw"] - w["trip_unit_mw"]) - w["limit_7pm_mw"]
    )
    pin(
        "own-station shortfall is derived in the data build, not typed here",
        w["own_gap_mw"] == missing,
    )
    has("own-station shortfall (paragraph + ledger)", f"{missing} MW")
    pin(
        "own-station shortfall appears in the paragraph, the title and the ledger",
        static.count(f"{missing} MW") >= 3,
    )
    has(
        "own-station size is named as a choice",
        f"{w['own_station_mw']:,.0f} MW station",
    )
    ...
    has(
        "marginal-hour count with added demand", f"in {mw_['top_hours']} of those hours"
    )
    pin(
        "the base case is coal in every hour",
        pr["marginal_base"]["top"] == "coal" and pr["marginal_base"]["top_hours"] == 24,
    )
    has(
        "headroom left in the stack",
    ...
    has(
        "the coal price names its regulator and resolution",
        "Resolution No. 10, Series of 2026",
    )
    pin(
        "the page never claims no price setter exists",
        "names no price setter" not in static,
    )
    pin(
        "the 770 MW is never credited to both routes at once",
        "two existing 230 kV routes deliver" not in flat
        and "two existing power routes can carry" not in flat,
    )

    # every source URL the data build carries must appear as a link on the page, so
    ...
    pin(
        f"all {len(urls)} generated source URLs are linked on the page",
        not unlinked,
        "; ".join(unlinked[:3]),
    )
    ...
    pin(
        "no chart leans on a methodology link instead of a real source",
    ...
    pin(
        f"all {len(cards)} cards footnote citable public sources",
        len(cards) == 9 and not bad,
        ", ".join(bad),
    )
    ...
    for m in re.finditer(r'<div class="fig[^"]*" id="(fig-[a-z]+)"', html):
        ...
        seen = re.sub(
            r"https?://\S+",
            " ",
            re.sub(r"\s+", " ", re.sub(r"<[^>]+>", " ", html[m.start() : stop])),
        )
    ...
    pin(
        "every acronym is spelled out on the card that uses it",
        not undefined,
        ", ".join(undefined),
    )
    ...
    pin(
        "no reader-facing line uses 'the campus' or 'the site' instead of its name",
        not vague,
        f"{len(vague)} left",
    )
    ...
    titles = [
        flat_title(m)
        for m in re.findall(r'<div class="figtitle">(.*?)</div>', html, re.S)
    ]
    ...
    if os.path.exists(doc_path):
        ...
        pin(
            f"all {len(titles)} chart titles appear verbatim as doc headings",
            len(titles) == 9 and not missing,
            "; ".join(missing[:2]),
        )
        pin(
            "doc headings carry the chart titles in the page's own order",
    ...
    pin(
        "no reader-facing line says a certificate has not been issued",
    ...
    orec = w["outages"]
    pin(
        "outage record: named units with measured days out",
        orec["days_in_window"] == p["archive_days"]
        and len(orec["units"]) >= 2
...
def main():
    ...
    pin(
        "archive peaks present for all three grids",
        all(k in pk for k in ("luzon", "visayas", "mindanao")),
    )
    pin(
        "campus exceeds Visayas archive peak",
        c["mw"] > pk["visayas"],
        f"{pk['visayas']}",
    )
    pin(
        "campus exceeds NGCP June Visayas available capacity",
        c["mw"] > p["visayas_bulletin"]["avail_hi"],
    )
    pin(
        "Luzon peak in a sane band (13-16 GW)",
        13000 < pk["luzon"] < 16000,
        f"{pk['luzon']}",
    )
    pin(
        "homes arithmetic: 72 GWh/day over 200 kWh/month",
        abs(
            p["homes_million"]
            - round(c["mw"] * 24 * 1000 / (p["home_kwh_month"] / 30.4166) / 1e6, 1)
        )
        < 0.05,
    )

    pin(
        "acwa covers 4-5% of the campus day (cloudless)",
        3.5 <= s["acwa_share_pct"] <= 5.0,
        f"{s['acwa_share_pct']}%",
    )
    pin(
        "sun-alone nameplate = demand over cloudless CF",
        abs(s["sun_alone_mw"] - round(c["mw"] / s["cf_cloudless"], -2)) < 1,
        f"{s['sun_alone_mw']}",
    )
    pin(
        "sun-alone land uses Terra's 1.0 ha/MW",
        abs(s["sun_alone_km2"] - round(s["sun_alone_mw"] / 100, 0)) < 1,
    )
    pin(
        "land comparisons consistent with areas",
        abs(s["vs_makati"] - round(s["sun_alone_km2"] / s["makati_km2"], 1)) < 0.05
        and abs(s["vs_ncc"] - round(s["sun_alone_km2"] / s["ncc_km2"], 2)) < 0.01,
    )
    pin(
        "night bridge vs Terra BESS ratio consistent",
        abs(s["vs_terra_bess"] - round(s["bridge_gwh"] * 1000 / 4500, 1)) < 0.05,
    )

    pin(
        "7pm line limit within the generated hourly band",
    ...
    # The page must not merge the two owners, so the data build keeps them apart.
    sub = w["substation"]
    pin(
        "substation is the announced P6.95B 230 kV end-2028",
        sub["php_b"] == 6.95 and sub["kv"] == 230 and sub["year"] == 2028,
    )
    pin(
        "the substation cost and the substation date carry different owners",
        sub["cost_owner"] == "NGCP transmission development plan"
        and sub["date_owner"] == "BCDA target",
    )

    pin(
        "price: modeled on the same recorded day as the wires limit",
        pr["day"] == w["day"],
    )
    pin(
        "price: with-campus Luzon mean is roughly double the base",
        1.5 <= pr["luzon_mean"][1] / pr["luzon_mean"][0] <= 3.0,
        f"{pr['luzon_mean']}",
    )
    pin(
        "price: campus raises the inter-island congestion rent",
        pr["links_rent_m_php"][1] > pr["links_rent_m_php"][0],
        f"{pr['links_rent_m_php']}",
    )
    pin(
        "price: DIPCEF stats carried as stated in methodology",
    ...
    mer = pr["merit"]
    pin(
        "merit ladder: blocks are cost-ordered and contiguous",
        all(
            mer["blocks"][i]["cost"] <= mer["blocks"][i + 1]["cost"]
    ...
    def landing(mw):
        return next(b for b in mer["blocks"] if b["cum_from"] < mw <= b["cum_to"])

    pin(
        "merit ladder: today's evening demand lands on the P6 coal block",
        math.isclose(landing(mer["evening_demand_mw"])["cost"], 6.0),
    )
    pin(
        "merit ladder: demand with the campus lands on the P12 oil block",
        math.isclose(landing(mer["demand_with_campus_mw"])["cost"], 12.0),
    )
    pin(
        "merit ladder: campus delta is the announced 3,000 MW",
        mer["demand_with_campus_mw"] - mer["evening_demand_mw"] == 3000,
    )

    pin(
        "perspective anchors: pools, own-gap homes, passthrough, island people",
        wa["pools_per_day"] == [round(65 / 2.5), round(90 / 2.5)]
        and abs(
            w["own_gap_homes_million"]
            - round(
                (c["mw"] - 1900 - w["limit_7pm_mw"])
                * 24
                * 1000
                / (p["home_kwh_month"] / 30.4166)
                / 1e6,
                1,
            )
        )
    ...
    pin(
        "solar: 24 hourly cover values, evening zero, midday sliver",
    ...
    pin(
        "water people range = MLD over 150 L/person",
        wa["people_equiv"] == [round(65e6 / 150, -3), round(90e6 / 150, -3)],
        f"{wa['people_equiv']}",
    )
    pin(
        "Makati multiple uses the 2024 census (post-Embo)",
    ...
    pin(
        "rice hectares come from the FAO paddy duty the cited page prints",
        wa["rice_l_s_ha"] == 1.5
        and wa["rice_ha"] == [round(65 / duty, -1), round(90 / duty, -1)],
        f"{wa['rice_ha']}",
    )
    pin(
        "Angat share stays a small percent (1-3%)",
    ...
    for section in (c, p, s, w, wa):
        ...
        break
    pin(
        "every section carries src URLs",
