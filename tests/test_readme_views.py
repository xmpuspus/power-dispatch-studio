...
The README lists all 39 studio views as `#v=<slug>` deep links instead of
embedding 39 clips, because GitHub applies no lazy loading and 39 clips would
cost about 200 MB on open. A hand-maintained list of 39 rows is the most
...
dests = destinations()
check("nav.ts declares 39 destinations", len(dests) == 39)
...
check(f"the table lists all 39 ({len(in_table)})", len(in_table) == len(declared))
