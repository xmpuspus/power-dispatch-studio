#!/usr/bin/env python3
"""Pins on the generated siting limits (web/data/sites.json).
...
check("at least one site is generated", d.get("n_sites", 0) > 0)
check("the count matches the list", d["n_sites"] == len(d["sites"]))
check("the note says the ratings are estimates", "rated to carry" in d.get("note", ""))
...
for s in d["sites"]:
    ...
    check(
        f"{tag}: reports how far it sits from the model",
    ...
    check(
        f"{tag}: min and max bracket the hours",
        not good
        or (
            s["limit_min_mw"] <= min(good) + 1e-6
    ...
    # assumption broke and all calculated site values changed with it.
    err = s.get("linearity_max_error")
    check(f"{tag}: solving at the limit sits on the rating", err is None or err < 0.01)
    check(f"{tag}: has at least one circuit", len(s.get("circuits", [])) > 0)
    check(
        f"{tag}: one outage row per circuit",
        len(s.get("outages", [])) == len(s["circuits"]),
    )
    check(
        f"{tag}: radial flag agrees with the outage rows",
        s["radially_fed"] == any(o["cuts_site_off"] for o in s["outages"]),
    )
    for o in s["outages"]:
        check(
            f"{tag}: an outage never leaves a negative limit",
        ...
        check(
            f"{tag}: a cut-off site reaches less than half its island",
            not o["cuts_site_off"]
    ...
    if s.get("already_over_rating"):
        check(
            f"{tag}: no headroom where a circuit is already over",
...
check("the Pax Silica site is generated", pax is not None)
if pax:
    ...
    check(
        "exactly one of its circuits cuts it off",
