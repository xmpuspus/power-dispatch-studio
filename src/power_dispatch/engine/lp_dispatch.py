# GENERATED from pipeline/lp_dispatch.py by tools/sync_engine.py -- do not edit.
...
def price_label(
    ...
    Shared verbatim by both engines; saved reference cases check it."""
...
def _highs_solve(lp_text: str) -> dict:
    ...
    try:
        ...
        sol = h.getSolution()
        lp = h.getLp()
        cols = {name: sol.col_value[i] for i, name in enumerate(lp.col_names_)}
        duals = {name: sol.row_dual[i] for i, name in enumerate(lp.row_names_)}
...
def _assemble(dispatch: dict, profiles: dict, date: str, opts: dict) -> dict:
    ...
    if offer_day:
        ...
        for h in range(24):
            for g in GRID_KEYS:
                ...
                o_demand[g].append(
                    day["demand"][g][h] + delta_at(opts.get("demand_delta"), g, h)
                )
        dearest = max(
            (b["cost"] for g in GRID_KEYS for hb in o_stacks[g] for b in hb),
            default=12.0,
        )
    ...
    storage = [
        {
            "grid": s["grid"],
            "power_mw": float(s["power_mw"]),
            "energy_mwh": float(s["energy_mwh"]),
            "eff": eff,
        }
    ...
    if day_budget:
        hydro_budget = {}
        for g in GRID_KEYS:
            base_hydro = dispatch["merit_order"][g]["fuel_avail_mw"].get("hydro") or 0.0
    ...
    dearest = max(
        (b["cost"] for g in GRID_KEYS for hb in stacks[g] for b in hb), default=12.0
    )
    voll = max(OFFER_CAP, dearest + 0.001)

    # a daily gas-energy limit for lower Malampaya supply, a
...
def run_chronology_lp(
    ...
    for h in range(24):
        ...
        for g in GRID_KEYS:
            ...
            marg[g] = price_label(
                price[g], cost, fuel, store_marg[g], hyd_marg, shed[g] > STORE_EPS
            )
...
def run_week_lp(
    dispatch: dict, profiles: dict, dates: list[str], opts: dict | None = None
) -> dict:
    ...
    stacks = {g: [h for m in days for h in m["stacks"][g]] for g in GRID_KEYS}
    demand = {g: [v for m in days for v in m["demand"][g]] for g in GRID_KEYS}
    caps = {
        "leyte": [c for m in days for c in expand(m["caps"]["leyte"])],
        ...
        day_summaries.append(
            {
                "date": dates[dd],
                "mean_price": {
                    g: round3(sum(o["price"][g] for o in block) / 24) for g in GRID_KEYS
                },
                "peak_price": {g: max(o["price"][g] for o in block) for g in GRID_KEYS},
    ...
    def rent_m_php(key: str, flow_key: str) -> float:
        return round3(
            sum(
                abs(o[flow_key]) * o[key]["rent"] / 1000.0
                for o in out_hours
                if o[key]["sat"]
            )
        )
    ...
    for h in range(H):
        ...
        phys += wheel * (
            cols.get(f"f1p_{h}", 0.0)
            + cols.get(f"f1n_{h}", 0.0)
            + cols.get(f"f2p_{h}", 0.0)
            + cols.get(f"f2n_{h}", 0.0)
        )
    ...
    summary = {
        ...
        "mean_price": {
            g: round3(sum(o["price"][g] for o in out_hours) / H) for g in GRID_KEYS
        },
        "peak_price": {g: max(o["price"][g] for o in out_hours) for g in GRID_KEYS},
        "unserved_mwh": {
            g: round1(sum(o["shortfall"][g] for o in out_hours)) for g in GRID_KEYS
        ...
        "leyte_rent_m_php": rent_m_php("leyte", "flow_lv"),
        "mvip_rent_m_php": rent_m_php("mvip", "flow_vm"),
        "soc_swing_mwh": round1(max((o["soc_mwh"] for o in out_hours), default=0.0)),
    }
    return {
        "hours": out_hours,
        "days": day_summaries,
...
def solve_snapshot_lp(stacks: dict, demand: dict, caps: dict, wheel: float) -> dict:
    ...
    dearest = max((b["cost"] for g in GRID_KEYS for b in stacks[g]), default=12.0)
    voll = max(OFFER_CAP, dearest + 0.001)
    ...
    sat1 = caps["leyte"] > FLOW_SAT_EPS and abs(f1) >= caps["leyte"] - FLOW_SAT_EPS
    sat2 = caps["mvip"] > FLOW_SAT_EPS and abs(f2) >= caps["mvip"] - FLOW_SAT_EPS
    rent1 = (
        round3(
            price["visayas"] - price["luzon"]
            if f1 > 0
...
if __name__ == "__main__":
    ...
    web = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "web", "data")
