# GENERATED from pipeline/chrono.py by tools/sync_engine.py -- do not edit.
# Edit the pipeline source and re-run the sync; tests/test_engine_sync.py enforces identity.
#!/usr/bin/env python3
"""Chronological dispatch reference engine used to check the browser calculation.
...
discharge-dear heuristic. This module implements the same calculation in Python
and consumes the same generated inputs. build_chrono_golden() emits input/output
pairs the studio browser test must reproduce, like dispatch.scenario_golden.

Still a simplified model: block dispatch per hour, no inter-temporal optimisation.
The storage rule charges below the day's 25th percentile price and discharges
above the 75th percentile. The thresholds come from a calculation without storage.

build_backcast() replays every market day with the base model (no storage cycling
or scenario changes) against the observed hourly LWAP and reports MAE, bias,
correlation, and high-hour hit rate. No parameters are fit to target prices, and
the remaining error is reported.
"""
...
# Outputs and rounding rules match studio/src/studio/chrono.ts. The agreement test
# fails if Python and the browser calculate different results.
EPS_SOC = 1e-9
...
# Stack and coupled clear over given stacks, mirrored in engine.ts.


def build_stack(
    fuel_avail: dict, removed: dict, added: list[dict], p: dict
) -> list[dict]:
    blocks: list[dict] = []
    for fuel, avail in fuel_avail.items():
        ...
        if fuel == "coal":
            ...
            blocks.append({"fuel": "coal", "cost": p["coal_commit"], "mw": must_run})
            blocks.append(
                {"fuel": "coal", "cost": p["coal_price"], "mw": round1(mw - must_run)}
            )
        else:
            blocks.append(
                {"fuel": fuel, "cost": p["costs"].get(fuel, 0.0), "mw": round1(mw)}
            )
    for b in added:
        if b["mw"] > 0:
...
def marginal(blocks: list[dict], g: float) -> tuple[float, str | None]:
    ...
    cost-dominance check keeps comparing the same problem."""
    if not blocks:
...
def clear_coupled_stacks(demand: dict, stacks: dict, caps: dict, wheel: float) -> dict:
    ...
    for _ in range(60):
        ...
        lo2 = max(-c2, nf1 - dV)
        hi2 = min(c2, dM)
        nf2 = _opt_flow(lambda x, nf1=nf1: mcM(x) - mcV(nf1, x), lo2, hi2, wheel)
...
def delta_at(delta: dict | None, grid: str, hour: int) -> float:
    ...
    if len(v) != 24:
        raise ValueError(
            f"demand_delta[{grid}] must be a number or 24 hourly values, got {len(v)}"
        )
    return float(v[hour])
...
def run_chronology(
    ...
    params = {
        "coal_commit": a["coal_commit_php_kwh"],
        "coal_min_frac": a["coal_min_load_frac"],
        "coal_price": costs["coal"],
        "costs": costs,
    }
    ...
    # problem to stay a valid independent cost check
    ...
    for g in GRID_KEYS:
        ...
        # independent reference for the cost-dominance test
        for fuel, dev in (out_dev.get(g) or {}).items():
            if fa.get(fuel) is not None:
                fa[fuel] = max(0.0, round1(fa[fuel] - dev))
        for fuel, delta in ((opts.get("fuel_avail_delta") or {}).get(g) or {}).items():
    ...
    if opts.get("reserve_deduction"):
        ...
        for g in GRID_KEYS:
            reserve_add[g] = round1(
                sum(
                    v
                    for k, v in (day_req.get(g) or mean_req.get(g) or {}).items()
                    if k != "Rd"
                )
            )  # Rd is regulation-DOWN, not withheld up-capacity
    ...
    def clear_hour(
        h: int, extra_demand: dict | None = None, added: dict | None = None
    ) -> dict:
        dem = demand_at(h)
        if extra_demand:
            for g, v in extra_demand.items():
                dem[g] = dem[g] + v
        stacks = {
            g: build_stack(fuel_avail_at(g, h), {}, (added or {}).get(g) or [], params)
            for g in GRID_KEYS
        }
        caps_h = {k: (v[h] if isinstance(v, list) else v) for k, v in caps.items()}
    ...
    for s in opts.get("storage") or []:
        ...
        if power <= 0 or energy <= 0:
            continue
        order = sorted(
            hours, key=lambda h, g=g: (pass1[h]["price"][g], pass1[h]["demand"][g], h)
        )
        ...
        stores.append(
            {"grid": g, "charge": charge, "discharge": discharge, "soc": soc_series}
        )
    ...
    for h in hours:
        if stores:
            ...
            for s in stores:
                ...
                if s["discharge"][h] > 0:
                    added[s["grid"]].append(
                        {"fuel": "storage", "cost": offer, "mw": s["discharge"][h]}
                    )
    ...
    summary = {
        "date": date,
        "mean_price": {
            g: round3(sum(o["price"][g] for o in out_hours) / 24) for g in GRID_KEYS
        },
        "peak_price": {g: max(o["price"][g] for o in out_hours) for g in GRID_KEYS},
        "unserved_mwh": {
            g: round1(sum(o["shortfall"][g] for o in out_hours)) for g in GRID_KEYS
        },
...
# Saved reference cases for the browser-to-Python comparison.
...
def build_chrono_golden(dispatch: dict, profiles: dict) -> dict:
    ...
    cases = [
        ...
        {
            "label": "default storage with the DICT demand forecast",
            "opts": {"demand_delta": {"luzon": 1500}, "storage": default_storage},
        },
    ...
    # per-day file (web/data/offers/) and must build identical text
    ...
    if os.path.isfile(offer_path):
        ...
        cases.append(
            {
                "label": "observed offer book, unchanged scenario",
                "opts": {"offer_mode": True},
            }
        )
        cases.append(
            {
                "label": "DICT 1.5 GW on the observed offer book",
                "opts": {"offer_mode": True, "demand_delta": {"luzon": 1500}},
            }
        )
        cases.append(
            {
                "label": "reserve withheld from the observed book",
                "opts": {"offer_mode": True, "reserve_deduction": True},
            }
        )
        ...
        out.append(
            {
                ...
                "expect": {
                    "price": {
                        g: [o["price"][g] for o in res["hours"]] for g in GRID_KEYS
                    },
                    "flow_lv": [o["flow_lv"] for o in res["hours"]],
                    "flow_vm": [o["flow_vm"] for o in res["hours"]],
                    "soc_mwh": [o["soc_mwh"] for o in res["hours"]],
                    "shortfall_luzon": [o["shortfall"]["luzon"] for o in res["hours"]],
                    # Save the exact label because rounded generation can flip the
                    # marginal block at a boundary without changing the price.
                    "marginal_luzon": [o["marginal"]["luzon"] for o in res["hours"]],
                    "summary": res["summary"],
                },
            }
        )
    # Saved 168-hour case: seven consecutive full-coverage days in one LP,
    # a battery and added demand so storage cycles and carries across
    ...
    full = [
        dd["date"]
        for dd in profiles["days"]
        if dd["market"]
        and all(
            len((dd.get("lwap") or {}).get(g) or []) == 24
    ...
    if len(full) >= 7:
        ...
        week_block = {
            "available": True,
            "label": "168-hour week with added demand and 2 GW / 16 GWh storage",
            "input": {"dates": week_dates, **week_opts},
            "lp_sha256": wr["lp_sha256"],
            "expect": {
                "price": {g: [o["price"][g] for o in wr["hours"]] for g in GRID_KEYS},
...
# Base model compared with recorded hourly LWAP over the market window.
...
def _score_pairs(pts: list[tuple[float, float]]) -> dict | None:
    ...
    hit_rate = round(100 * hit / len(high), 1) if high and rankable else None
    return {
        "n_hours": n,
    ...
    for key, label in (("lv", "Luzon to Visayas"), ("vm", "Visayas to Mindanao")):
        ...
        out[key] = {
            ...
            "direction_agreement_pct": (
                round(100 * agree / len(decisive), 1) if decisive else None
            ),
            "n_decisive_hours": len(decisive),
            "observed_binding_share_pct": (
                round(100 * bind_y / bind_n, 1) if bind_n else None
            ),
            "modeled_at_cap_share_pct": (
                round(100 * at_cap / n_cap, 1) if n_cap else None
            ),
        }
    return out or None
...
def build_offer_backcast(profiles: dict) -> dict:
    ...
    the same three targets as the cost-based historical replay, so the two sit side
    by side and show the offer premium directly."""
    ...
    files = {
        ...
        for p in _glob.glob(
            os.path.join(here, "..", "data", "derived", "offer_daily", "OFFERD_*.json")
        )
    }
    if not files:
        return {
    ...
    for day in profiles["days"]:
        ...
        for key in ("leyte", "mvip"):
            ...
            if frac:
                o_caps[key] = [round1(o_caps[key] * frac[h]) for h in range(24)]
        text = build_day_lp(
            stacks, demand, o_caps, 0.02, [], None, max(OFFER_CAP, dearest + 0.001)
        )
    ...
    per_grid = {g: _score_pairs(pairs[g]) for g in GRID_KEYS if _score_pairs(pairs[g])}
    per_grid_mcp = {
        g: _score_pairs(pairs_mcp[g]) for g in GRID_KEYS if _score_pairs(pairs_mcp[g])
    }
    flows = {}
    for key, label in (("lv", "Luzon to Visayas"), ("vm", "Visayas to Mindanao")):
        pts = flow_pairs[key]
    ...
    return {
        ...
        "days": len(days_used),
        "window": ({"from": days_used[0], "to": days_used[-1]} if days_used else None),
        "per_grid": per_grid,
        "per_grid_mcp": per_grid_mcp or None,
        "flows": flows or None,
        "flows_rtdhs": _flows_vs_record(day_flows),
        "flows_rtdhs_note": (
            "The same modeled flows scored against the "
        ...
        "note": (
            "The same replays with the operator's own OFFER BOOKS "
            ...
            "is the model-derived offer premium for each hour."
        ),
    }


def build_backcast(
    dispatch: dict, profiles: dict, observed_solar: bool = False
) -> dict:
    ...
    scenario changes, with no parameters fit to target prices. The remaining
    error is reported.
    ...
    (vre_probe): it is off in the published historical replay because it worsens
    the price correlation (the flat shape misplaces the observed energy without an
    ...
    for c in dispatch["coupling"]["corridors"]:
        base_caps["leyte" if c["id"] == "leyte_luzon_hvdc" else "mvip"] = c["limit_mw"]
    days_used = []
    for day in profiles["days"]:
        if not day["market"]:
            continue
        ...
        for key, ck in (("lv", "leyte"), ("vm", "mvip")):
            frac = cc.get(ck)
            rec["cap_" + key] = (
                [round1(base_caps[ck] * frac[h]) for h in range(24)]
                if frac
                else [base_caps[ck]] * 24
            )
        day_flows[day["date"]] = rec
        for g in GRID_KEYS:
            mcg = mc.get(g) or []
        ...
        for key, mod_key in (("lv", "flow_lv"), ("vm", "flow_vm")):
            ...
            for h in range(24):
                if h < len(obs_series) and obs_series[h] is not None:
                    flow_pairs[key].append((res["hours"][h][mod_key], obs_series[h]))
    per_grid = {}
    per_grid_mcp = {}
    for g in GRID_KEYS:
    ...
    return {
        ...
        "flows_note": (
            "Modeled corridor flows scored against the observed "
        ...
        "mcp_note": (
            "per_grid_mcp scores the same replays against the "
        ...
        "note": "Every market day with complete data is recalculated using recorded "
        "hourly demand and the base plant and grid settings. Storage and user scenario "
        "changes are excluded. The model inputs are not adjusted to match prices. "
        "Differences from recorded prices can reflect scarcity, plant offers, price "
        "caps, outages, and other conditions that are not present in the model.",
    }
